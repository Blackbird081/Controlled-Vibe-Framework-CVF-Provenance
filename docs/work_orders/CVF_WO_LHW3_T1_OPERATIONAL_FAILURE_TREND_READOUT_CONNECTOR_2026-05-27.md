...
Closes the gap where W4 reports per-call metrics and V3 classifies 22 diagnostic
classes, but no connector standard ties recurring failure patterns into a trend
signal with recommended operator action.
...
4. `docs/reviews/CVF_WR1_WORKFLOW_RECOVERY_STATE_PROOF_COMPLETION_2026-05-25.md`
   — confirm V3 diagnostic class vocabulary and recovery readout structure;
   note which diagnostic classes map to overconstraint vs instability
5. `docs/reference/CVF_LHW2_MEMORY_EVENT_CAPTURE_WORKFLOW_RECEIPT_LOOP_CONNECTOR_SPEC_2026-05-27.md`
   — understand the S2/S5 table pattern for field mapping and boundary table
6. `docs/reference/CVF_LHW3_WORKFLOW_CONNECTOR_WAVE3_ROADMAP_2026-05-27.md`
...
- High `policyViolationRate` + `policy_enforcement_rejection` class → overconstraint signal → review policy / payload boundaries
- High `retryCount` + `model_unavailable` or `provider_timeout` class → instability signal → check provider health / rotate provider
- Low `taskCompletionRate` + `spec_ambiguity_clarification_required` class → underspecification signal → request clarification (see T2 loop)
- High `humanCorrectionCount` (any class) → drift signal → review prompt engineering / pack selection
- Low `receiptIntegrityRate` + `receipt_missing_or_invalid` class → audit gap signal → inspect receipt chain

Use W4 metric field names and V3 diagnostic class tokens verbatim. If a field
name cannot be verified from the W4 completion review or WR1 completion review,
mark it `BLOCKED_SOURCE_NOT_FOUND`, stop the work order, and return to
Orchestrator. Do not carry `UNVERIFIED`, `TBD`, or "confirm later" wording
into the spec.
...
return to Orchestrator. No blocked, guessed, inferred, or confirm-later item
may remain in S2.
...
- [ ] W4 metric field names confirmed from W4 completion review
- [ ] V3 diagnostic class tokens confirmed from WR1 completion review
...
2. Confirm W4 metric names and V3 class tokens from completion reviews.
...
- a W4 metric field name or V3 diagnostic class token cannot be confirmed
  from the completion reviews without modifying source;
...
source-verified W4 and V3 field names from existing completion reviews; no
