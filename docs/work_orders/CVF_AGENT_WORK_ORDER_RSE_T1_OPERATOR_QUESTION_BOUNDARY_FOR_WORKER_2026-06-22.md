# CVF Agent Work Order - RSE-T1 Operator Question Boundary
...
Current-time notes: RSE-T0 is closed (`c0664784`, sync `ad365c43`). The roadmap
...
uncommitted work for the reviewer/closer. RSE-T1 is documentation only.

Do-not-misread notes: implement only the RSE-T1 documentation addendum. Do not
implement any checker, helper, or AAF diagnostic; do not edit `governance/compat/`
files or dispatch-quality checker code; do not edit the closed RSE-T0 standard or
any existing governed artifact; do not change AHB-T2 or AHB-T3 contract
semantics; do not author RSE-T2 or RSE-T3 content; do not add runtime,
provider/live, public-sync, CLI/MCP adapter, workspace, queue, daemon, watcher,
direct interception, readiness, or universal control behavior or claim.

Required first actions: read this work order, read the RSE-T1 GC-018 baseline,
read the RSE roadmap T1 section, read the closed RSE-T0 standard, read the Guard
Orientation Index role glossary, read the AAF-T7B completion review learning
capture decision, confirm actual `executionBaseHead`, and inspect current
`git status --short`.
...
| Field | Disposition |
|---|---|
| Intake source | operator instruction to close RSE-T0 and open RSE-T1 Operator Question Boundary |
| Intake role | worker creates the RSE-T1 documentation addendum |
| Reviewer role | reviewer/closer validates question-class coverage, the forbidden pattern, the routing rules, and claim boundary |
...
| escalation condition | stop and return `BLOCKED_WITH_REASON` if implementation would require any checker or helper code, edit to the closed RSE-T0 standard, AHB semantics change, RSE-T2/T3 content, runtime/provider/live, public-sync, session/handoff edits, or forbidden path scope |
...
| baseHeadFor(phase) | `dispatchBaseHead=ad365c43`; `executionBaseHead` recorded by worker; `closureBaseHead` set by reviewer before closure commit |
...
| traceScope(phase, actor) | one worker-return trace covers pending RSE-T1 artifacts; one reviewer trace covers review/closure |
...
| crossBatchIsolation | do not mix RSE-T1 with RSE-T0 edits, RSE-T2/T3, AAF, MPI, runtime/provider/live, MCP adapter, public-sync, queue/daemon, direct-interception, generated aggregate, or memory-plane work |
| Before status evidence | dispatchBaseHead `ad365c43`; clean worktree verified before RSE-T1 dispatch authoring |
...
| Field | Disposition |
|---|---|
| completionReviewPath | `docs/reviews/CVF_RSE_T1_OPERATOR_QUESTION_BOUNDARY_COMPLETION_2026-06-22.md` |
...
| `docs/baselines/CVF_GC018_RSE_T1_OPERATOR_QUESTION_BOUNDARY_2026-06-22.md` | GC-018 authorization and claim boundary |
| `docs/roadmaps/CVF_RSE_ROLE_SWITCH_ENVELOPE_PROTOCOL_ROADMAP_2026-06-22.md` | roadmap T1 question classes and acceptance criteria |
| `docs/reference/role_switch_envelope/CVF_ROLE_SWITCH_ENVELOPE_STANDARD.md` | the closed RSE-T0 standard the addendum extends |
...
`BLOCKED_WITH_REASON` if a required command cannot run. RSE-T1 adds no Python
...
public/session scope, require AHB semantics change, require an edit to the closed
RSE-T0 standard, require RSE-T2/T3 content, touch forbidden paths, consume
...
return, not escalated to the operator. This rule is itself an instance of the
RSE-T1 boundary the addendum documents.
...
3. Define the four canonical question classes (`ASK_OPERATOR`,
   `ASK_REVIEWER_OR_CLOSER`, `SELF_HANDLE_WITHIN_SCOPE`,
   `RETURN_BLOCKED_WITH_REASON`) with the decision examples from the roadmap T1
   section.
4. Represent the AAF-T7B confusing question as a forbidden pattern and classify
   its parts: finding capture is `SELF_HANDLE_WITHIN_SCOPE`; promotion routing is
   `ASK_REVIEWER_OR_CLOSER`; neither part is `ASK_OPERATOR`.
5. State that finding capture is mandatory inside the allowed return surface and
   that out-of-scope promotion is routed and not performed.
6. State that the addendum is documentation only, builds on the closed RSE-T0
   standard, and is a local view of the Agent Handoff Contract.
7. Run required checks and create the worker-return artifact without committing.

## Write Ownership

| Path | Owner during worker execution | Disposition |
|---|---|---|
| `docs/reference/role_switch_envelope/CVF_RSE_T1_OPERATOR_QUESTION_BOUNDARY_ADDENDUM.md` | worker | create the RSE-T1 documentation addendum |
| `docs/reviews/CVF_RSE_T1_OPERATOR_QUESTION_BOUNDARY_WORKER_RETURN_2026-06-22.md` | worker | create |
| `docs/baselines/CVF_GC018_RSE_T1_OPERATOR_QUESTION_BOUNDARY_2026-06-22.md` | reviewer/closer | no worker edit |
| `docs/work_orders/CVF_AGENT_WORK_ORDER_RSE_T1_OPERATOR_QUESTION_BOUNDARY_FOR_WORKER_2026-06-22.md` | reviewer/closer | no worker edit |
| `docs/reference/role_switch_envelope/CVF_ROLE_SWITCH_ENVELOPE_STANDARD.md`, `docs/reference/role_switch_envelope/README.md` | not worker-editable | forbidden: closed RSE-T0 artifacts |
| `governance/compat/**`, dispatch-quality checker, AHB contract files, `AGENTS.md`, `CVF_SESSION/**`, active handoff, runtime routes, web, MCP, provider registry, generated aggregates, public-sync, queue/daemon/watcher, RSE-T2/T3 files | out of worker scope | forbidden |
...
checker or helper implementation, an edit to the closed RSE-T0 standard, AHB
semantics change, RSE-T2/T3 content, runtime/product behavior, generated
aggregate edits, AGENTS/session/handoff edits, provider/live/public-sync scope,
...
classes, marks the AAF-T7B confusing question as forbidden, and states that
...
| Roadmap author | owns the RSE T0-T3 roadmap |
| Dispatcher | owns the GC-018 baseline and work order |
| Work-order reviewer | reviews and accepts the packet before execution |
| Worker | RSE-T1 addendum author role |
...
|---|---|---|
| Operator instruction | 2026-06-22 close RSE-T0; open RSE-T1 with fresh GC-018 and source-verified work order | ACCEPT |
| RSE-T1 GC-018 | `docs/baselines/CVF_GC018_RSE_T1_OPERATOR_QUESTION_BOUNDARY_2026-06-22.md` | ACCEPT |
...
- create `docs/reference/role_switch_envelope/CVF_RSE_T1_OPERATOR_QUESTION_BOUNDARY_ADDENDUM.md`
  with the four canonical question classes, the AAF-T7B forbidden pattern, the
  mandatory finding-capture rule, the route-do-not-perform promotion rule, and an
...
- no checker, helper, or AAF diagnostic implementation and no edit to
  `governance/compat/` files or dispatch-quality checker code;
- no edit to the closed RSE-T0 standard, its front door, or any existing governed
  artifact;
- no change to AHB-T2 or AHB-T3 contract semantics;
- no RSE-T2 or RSE-T3 content;
...
|---|---|---|---|
| RSE-T1 four question classes | roadmap line 196 | addendum defines all four classes | source diff and worker return |
| RSE-T1 AAF-T7B forbidden pattern | roadmap line 214 | addendum encodes the AAF-T7B pattern | source diff |
| RSE-T1 finding-capture-mandatory rule | roadmap line 215 | addendum states finding capture is mandatory | source diff |
| RSE-T1 promotion-routed rule | roadmap line 217 | addendum states promotion is routed, not performed | source diff |
| RSE-T2/T3 | roadmap lines 220, 259 | excluded from this tranche | claim boundary and no T2/T3 content |
...
|---|---|---|---|---|---|
| A worker raised a reviewer/closer decision to the operator | OPERATOR_SCOPE_CLARITY_GAP | GOVERNANCE_CONTROL_PLANE | WRITTEN_RULE_CANDIDATE | RSE-T1 addendum classifies questions and marks the AAF-T7B pattern forbidden | handled by this dispatch |
...
- Output traceability: RSE-T1 work order maps the RSE roadmap T1 section to
...
the stated role model. This work order authorizes only RSE-T1 documentation.
...
- explicit statement that the addendum defines all four question classes;
- explicit statement that the addendum encodes the AAF-T7B forbidden pattern and
  the mandatory finding-capture and routed-promotion rules;
- explicit statement that the addendum is documentation-only, builds on the
  closed RSE-T0 standard, and is an AHB local view;
- explicit statement that no checker, helper, AAF diagnostic, RSE-T0 edit, AHB
  semantics, RSE-T2/T3 content, runtime, provider/live, public-sync, generated
...
verify the addendum covers all four question classes and the required forbidden
pattern and routing rules, verify the documentation-only and AHB-local-view
boundary, verify the closed RSE-T0 files are unchanged, and only then convert
accepted material into a completion review.
...
checker/helper/RSE-T0-edit/AHB-semantics/RSE-T2-T3/runtime/provider/public/session
scope.
...
|---|---|---|
| AC1 | The addendum includes the four canonical question classes or a source-verified equivalent | source diff and worker return |
| AC2 | The addendum represents the AAF-T7B confusing question as a forbidden pattern | source diff |
| AC3 | The addendum states that finding capture is mandatory inside the allowed return surface | source diff |
...
| Before status evidence | dispatchBaseHead `ad365c43`; clean worktree verified before RSE-T1 dispatch authoring |
| After status evidence | RSE-T1 worker artifacts pending no-commit return |
...
This work order authorizes only the RSE-T1 documentation-only Operator Question
Boundary addendum plus a worker return. It does not authorize any checker,
helper, or AAF diagnostic implementation, any edit to the closed RSE-T0 standard
or any existing governed artifact, any change to AHB-T2 or AHB-T3 semantics, any
RSE-T2 or RSE-T3 content, runtime behavior, provider/live behavior, CLI/MCP
adapter behavior, public-sync, session-sync by the worker, direct interception,
readiness claims, speed/cost claims, or universal governed-coding control.
