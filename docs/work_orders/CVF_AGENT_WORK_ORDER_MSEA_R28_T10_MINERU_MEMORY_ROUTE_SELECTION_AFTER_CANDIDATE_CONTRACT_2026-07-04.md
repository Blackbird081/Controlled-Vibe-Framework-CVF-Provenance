# CVF Agent Work Order - MSEA R28 T10 MinerU Memory Route Selection After Candidate Contract
...
Worker return path: `docs/reviews/CVF_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_WORKER_RETURN_2026-07-04.md`

## Dispatch Prompt Envelope

Role: delegated worker for MSEA-R28-T10.

Canonical packet: `docs/work_orders/CVF_AGENT_WORK_ORDER_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_2026-07-04.md`

Commit mode: WORKER_MUST_NOT_COMMIT.

executionBaseHead: WORKER_MUST_CAPTURE_AT_START.

Current-time notes: artifact date is 2026-07-04. R28-T9 material commit
`45fb7a9a` accepted the deterministic metadata-only memory-safe candidate
contract helper, and session-sync commit `bd2c2d95` routes the next allowed
move to T10 route-selection work-order authoring.

Do-not-misread notes: this packet authorizes only docs-only route-selection
...
| Field | Disposition |
| --- | --- |
| Intake source | accepted R28-T9 closure evidence and active nextAllowedMove |
| Intake role | docs-only worker creates route-selection matrix and worker return |
| Scope classification | bounded documentation decision; no runtime/provider/public/private-content/memory-write behavior |
...
Create a source-verified T10 matrix that selects the next bounded route after
accepted T9 memory-safe candidate contract evidence. The expected decision
space is: candidate ready for future memory-owner review, continue full hold,
or reject memory route release; direct memory/RAG write remains unauthorized
unless a fresh memory-owner work order later releases it.
...
| --- | --- | --- |
| Operator instruction | operator asked to complete T7-T10 after T8/T9 progression | ACCEPT |
| Active session state | `CVF_SESSION/ACTIVE_SESSION_BOOTSTRAP_READ_MODEL.json` and `CVF_SESSION/ACTIVE_SESSION_STATE.json` route next allowed move to R28-T10 packet authoring only | ACCEPT |
| Active handoff | `AGENT_HANDOFF_V36_2026-07-04.md` names the same next allowed move and parked checkpoints | ACCEPT |
| R28-T9 closure | material commit `45fb7a9a` accepted the candidate contract helper and preserved memory write hold | ACCEPT |
| Paired GC-018 | `docs/baselines/CVF_GC018_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_2026-07-04.md` | ACCEPT |

## Dependency Release Evidence

| Dependency | Evidence | Disposition |
| --- | --- | --- |
| R28-T9 material closure | `docs/reviews/CVF_MSEA_R28_T9_MINERU_MEMORY_SAFE_CANDIDATE_CONTRACT_WORKER_RETURN_2026-07-04.md` and material commit `45fb7a9a` | ACCEPT |
| R28-T9 session-sync routing | `CVF_SESSION/ACTIVE_SESSION_BOOTSTRAP_READ_MODEL.json` at commit `bd2c2d95` | ACCEPT |
| Current dispatch base | `git rev-parse --short HEAD` returned `bd2c2d95` before authoring | ACCEPT |
| Worker execution release | R28-T10 dispatch artifacts and gates must exist before worker begins; this packet supplies that release after commit | ACCEPT |

## Scope / Target / Owner Boundary

Allowed scope:

- create `docs/reference/CVF_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_MATRIX_2026-07-04.md`;
- create `docs/reviews/CVF_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_WORKER_RETURN_2026-07-04.md`;
- select a docs-only route disposition after accepted T9 evidence;
- preserve direct memory/RAG write hold unless the matrix explicitly states a future memory-owner work order is required before any write.
...
Risk ceiling: R1 deterministic local documentation route selection; no runtime/provider/private-data/public action.
...
| T10 companion matrix | worker may create listed file only | WORKER_MUST_NOT_COMMIT |
| T10 worker return | worker creates, reviewer commits if accepted | WORKER_MUST_NOT_COMMIT |
| source/test/checker/hook/session files | not worker-owned in R28-T10 | forbidden |
...
| Confirm planned worker paths | `Test-Path` for matrix and worker return paths | no conflicting existing R28-T10 worker artifact |
...
| 3 | Create companion route-selection matrix from T8/T9/R27/R24 evidence | matrix diff |
| 4 | Select a bounded next route and state memory-write hold | matrix and worker return |
| 5 | Create worker return summarizing changed files, gates, no-commit status, and next-route recommendation | worker return |
...
| manualEditsAfterScaffold | filled source verification, dependency release, docs-only matrix scope, worker output manifest, checker-output read-ahead mandate, handoff control, and held memory-write boundary |
...
| Source contradiction blocks route selection | return `BLOCKED_WITH_REASON` with exact source conflict |
...
| --- | --- | --- |
| Planned T10 paths absent before authoring | `Test-Path` returned `False` for planned T10 baseline, work order, worker return, and matrix paths. | PASS |
| Token search for T10 before authoring | `rg -n "MSEA-R28-T10|MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT|Memory Route Selection After Candidate Contract" docs CVF_SESSION governance EXTENSIONS` found no existing T10 dispatch artifact before authoring. | PASS |
| Source helper freshness | `rg -n "MineruMemorySafeCandidateContract|build_mineru_memory_safe_candidate_contract|mineru_memory_safe_candidate_contract_payload|memoryWriteAuthorized|memoryWriteDisposition" ...` confirmed current T9 helper/test evidence. | PASS |
| Freshness disposition | Current source has T9 candidate contract evidence; no T10 route-selection matrix exists yet. | PASS |
...
| Exact search command or query | `rg -n "MSEA-R28-T10|MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT|Memory Route Selection After Candidate Contract" docs CVF_SESSION governance EXTENSIONS` | PASS |
...
| Same-token collision result | No dedicated T10 baseline, work order, worker return, or companion matrix existed before authoring. | PASS |
| Absent-versus-collision disposition | T8/T9 route recommendation references are predecessor evidence only; T10 may dispatch as a new docs-only route-selection worker packet. | PASS |

## Roadmap-To-Work-Order Trace Matrix

| Predecessor requirement | Source evidence | R28-T10 handling |
| --- | --- | --- |
| R28-T8 reserves T10 for route selection after T9 evidence | T8 matrix lines 59-63 | authorize docs-only route-selection matrix |
| R28-T9 supplies candidate contract helper evidence | T9 worker return and state entry | matrix may select candidate-ready-for-memory-owner-review |
| R27 requires future memory-owner work order before memory write | R27 ledger lines 86-87 | direct memory/RAG write remains unauthorized |
...
| --- | --- | --- | --- | --- | --- |
| `INTERNAL_AGENT` | governed docs-only route-selection matrix and worker return | internal worker may create documentation evidence under WORKER_MUST_NOT_COMMIT; reviewer must accept before closure | this work order, paired GC-018, source verification, worker return, governance gates | docs-only; no memory adapter or runtime action | ROUTE_SELECTION_ONLY |
| `EXTERNAL_AGENT_CLI_MCP` | no CLI/MCP or external adapter owner in R28-T10 | external ingress, authentication, approval, receipt, raw-data, mutation, and public boundaries remain out of scope | no source-verified external adapter authority in R28-T10 | adapter deferred; no CLI/MCP surface implemented or claimed | DEFERRED_WITH_REASON |
...
| Owner surface | paired R28-T10 GC-018 and this work order |
| Disposition | ADAPT accepted R28-T9 evidence into a bounded route-selection dispatch; no external source becomes authority by itself |
...
| Stable location decision | companion matrix belongs under `docs/reference/` because it is selected source-verified route evidence |
...
| phase | dispatch |
| baseHeadFor(phase) | dispatchBaseHead=bd2c2d95; executionBaseHead=WORKER_MUST_CAPTURE_AT_START; closureBaseHead=REVIEWER_TO_SET |
| changedSetScope(phase) | worker may create only the named companion matrix and named worker return |
| traceScope(phase, actor) | worker records executionBaseHead, changed files, worker-return gate, autorun evidence, route-selection disposition, and no-commit statement; reviewer records closure evidence |
| commitOwner(phase) | WORKER_MUST_NOT_COMMIT; reviewer/closer owns material commit if accepted |
| crossBatchIsolation | do not mix R28-T10 with memory-route write, runtime, provider/live, public-sync, app/use-case/legal, checker/hook edits, source/test edits, or session-sync work |
...
| Field | Value |
| --- | --- |
| completionReviewPath | `docs/reviews/CVF_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_COMPLETION_2026-07-04.md` optional; prefer repairing evidence in the worker return unless reviewer needs a separate closure packet |
...
| --- | --- |
| `docs/reference/CVF_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_MATRIX_2026-07-04.md` | create source-verified route-selection matrix and selected disposition |
| `docs/reviews/CVF_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_WORKER_RETURN_2026-07-04.md` | create no-commit evidence packet with changed files, gates, selected route, and no-memory-write boundary |

## Worker Return Packet Shape Contract

workerReturnPath: `docs/reviews/CVF_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_WORKER_RETURN_2026-07-04.md`
...
| --- | --- |
| companion matrix | exact path, selected route, source-backed rationale, and no-memory-write boundary |
...
| --- | --- |
| Route matrix | source-verified matrix selects a bounded next route after T9 evidence |
| Memory hold | direct memory/RAG write remains unauthorized unless matrix returns BLOCKED with source-backed contradiction |
...
- implementation requires running MinerU, provider/live proof, memory/RAG, public-sync, app work, source/test/checker/hook edits, or dependency installs;
- matrix claims direct memory write authorization without a fresh memory-owner work order;
...
python governance/compat/run_agent_autorun_workflow_gate.py --phase pre-implementation --base bd2c2d95 --head HEAD
git status --short --untracked-files=all
```

If the changed range differs from `bd2c2d95..HEAD` during worker execution,
capture the worker's executionBaseHead and run gates over the real worker
changed range.

## Operator Checkpoint

operator.checkpoint.waiver: Operator requested completion of T7-T10; T10 worker
execution remains bounded by this source-verified work order and returns
uncommitted for reviewer closure. Any memory write remains held for a future
memory-owner work order.
...
| Target paths | `docs/baselines/CVF_GC018_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_2026-07-04.md`; `docs/work_orders/CVF_AGENT_WORK_ORDER_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_2026-07-04.md` |
...
| Approval boundary | operator requested T7-T10 completion; this dispatch authorizes only T10 docs-only worker packet |
...
| Invocation ID | `msea-r28-t10-dispatch-2026-07-04` |
| Expected manifest | `docs/baselines/CVF_GC018_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_2026-07-04.md`; `docs/work_orders/CVF_AGENT_WORK_ORDER_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_2026-07-04.md` |
| Actual changed set | `docs/baselines/CVF_GC018_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_2026-07-04.md`; `docs/work_orders/CVF_AGENT_WORK_ORDER_MSEA_R28_T10_MINERU_MEMORY_ROUTE_SELECTION_AFTER_CANDIDATE_CONTRACT_2026-07-04.md` |
...
| Field | Value |
| --- | --- |
| claimScope | R28-T10 docs-only memory route selection work order |
...
| receiptEvidence | CLAIM_REJECTED_NO_RECEIPT: predecessor metadata/helper evidence is cited, but this work order creates no runtime receipt. |
| actionEvidence | CLAIM_REJECTED_NO_ACTION: this work order runs no MinerU, provider, memory, or external action. |
| invocationBoundary | local source verification, docs-only route-selection matrix, worker return, and governance gates only |
| interceptionBoundary | No direct interception, wrapper/proxy enforcement, runtime gate, or agent coding control is authorized. |
| claimLanguage | docs-only route-selection worker dispatch and bounded evidence only |
...
docs-only route-selection matrix and worker return after accepted T9 candidate
contract evidence. It does not authorize MinerU runtime execution,
...
Reason: R28-T10 is private provenance implementation-dispatch work and does not
