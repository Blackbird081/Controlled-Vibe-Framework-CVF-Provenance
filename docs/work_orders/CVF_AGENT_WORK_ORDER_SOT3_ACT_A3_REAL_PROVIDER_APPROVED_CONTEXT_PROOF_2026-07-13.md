...
| hard ceiling | two total DashScope calls for A3; a third call returns to operator |
...
- [ ] exactly five worker outputs and no existing runtime file mutation;
- [ ] real Alibaba `qwen-turbo` call succeeds through `/api/execute`;
- [ ] exactly one planned provider request, or one diagnosed retry within the
  two-call ceiling;
- [ ] `ENFORCE` provider system message contains Flow-approved context;
- [ ] durable record is re-read and correlated with governance receipt ID and
  envelope ID in the A3 receipt;
- [ ] full trace identifiers and acknowledged lifecycle are present;
- [ ] receipt `overall=PASS` only when all proof assertions pass;
- [ ] canonical evidence manifest hash matches the receipt;
- [ ] no raw key, bearer header, raw provider body/output, full prompt, or
  source content is committed;
- [ ] required local checks, typecheck, fast gate, diff hygiene, and no-commit
  evidence pass.
...
- [ ] execution base and clean status captured before edits;
- [ ] five outputs only and no forbidden mutation;
- [ ] planned live call count and any retry diagnostic recorded;
- [ ] receipt generated by live execution, not handwritten PASS;
- [ ] receipt-manifest hash verified;
- [ ] secret safety scan passed;
- [ ] focused regression, typecheck, and worker-return fast gate passed;
- [ ] HEAD unchanged and worker did not commit;
- [ ] worker returns one terminal disposition.
...
| Approval boundary | packet authoring and dispatch; worker implementation remains separate |
