# CVF Agent Work Order - SOT3-APP-T1-R1 Contract Ratification Completeness And Binding Correction
...
Worker return path: `docs/reviews/CVF_SOT3_APP_T1_R1_WORKER_RETURN_2026-07-17.md`
...
Correct all six consolidated T1 review findings in one direct-source-backed
ratification artifact before any later application mutation is authorized.

## Authority Chain

Operator standing continuation instruction -> SOT3-APP roadmap -> T0B closure
`577237cba` -> MAO-OA closure `fef756a14` -> original T1 review commit
`ef9b09648` -> paired T1-R1 GC-018 -> this work order.
...
| original T1 review | `docs/reviews/CVF_SOT3_APP_T1_COMPLETION_REVIEW_2026-07-17.md`; material commit `ef9b09648`; `REVIEWED_NOT_ACCEPTED_R1_REQUIRED` | one correction packet must resolve all six consolidated findings | ACCEPT |
| clean dispatch base | `3cf20ecce`; `git status --short` empty before authoring | no cross-batch residue | ACCEPT |
| T2 release | explicitly excluded | T1-R1 must close through independent review first | ACCEPT |
...
Query: `python governance/compat/run_adif_defect_resolver.py --task-class "downstream contract ratification correction" --role worker --lifecycle-phase pre-implementation --json`
...
|---|---|
| `docs/reviews/CVF_SOT3_APP_T1_R1_CONTRACT_RATIFICATION_COMPLETENESS_AND_BINDING_CORRECTION_2026-07-17.md` | create complete source-verified ratification and compatibility design |
| `docs/reviews/CVF_SOT3_APP_T1_R1_WORKER_RETURN_2026-07-17.md` | create exact two-path no-commit worker return |
...
1. Capture clean base, output absence, and accepted T0B/T1-review continuity.
2. Define exact search commands, patterns, exclusions, and two denominators:
   the full contract-bearing inventory and the decision-consuming subset.
3. Reconcile every unique returned file to one terminal row with zero
   unresolved; do not reuse the unreproducible original 32-row denominator.
4. Map all eight local adapters exported by `packages/cvf-bindings/src/index.ts`
...
6. Ratify fail-closed continuation: `ALLOW` maps to `CONTINUE`; `WARN` may map
   ...
   `REVIEW_REQUIRED`, and any unproven semantic map to `HOLD_FOR_REVIEW`;
   `BLOCK` maps to `STOP`.
7. Separate current source-visible fail-open behavior from the desired
   ratified contract, include the Kernel workflow consumer edge, and keep the
   Kernel vocabulary distinct.
8. Record exact checker commands/results without claiming gate chronology that
   the command evidence cannot prove; leave exactly two uncommitted paths.

## Consolidated T1-R1 Correction Mandate

| Finding | Required terminal correction | Fail condition |
|---|---|---|
| unreproducible denominator | exact commands, patterns, exclusions, unique-file totals, full inventory, decision subset, zero unresolved | unexplained count or omitted returned file |
| incomplete adapter coverage | all eight local adapters, binding-health, and principal callers mapped to current owners | deferred or omitted adapter/caller |
| identity/hash conflation | retain ID/reference and add/validate hash separately on both canonical shapes | hash replaces ID/reference |
| unsafe `ESCALATE` continuation | `HOLD_FOR_REVIEW` unless direct authority proves a safer explicit alternative | automatic continuation without authority |
| omitted Kernel workflow edge | include `refinery-to-kernel.workflow.ts` plus every decision consumer | adapter-only consumer claim |
| contradictory gate chronology | exact command evidence and bounded chronology claim | unsupported pre-write or first-discovery statement |

## Required Ratification Structure

The main artifact must contain: Executive Disposition; Authority And Source
Snapshot; Contract-Bearing Path Inventory; Business Domain Owner Map; Local
Adapter To Current CVF Public Surface Matrix; T8 Packet Binding Compatibility
Design; Five-Value Continuation Matrix; Kernel Decision Separation Matrix;
Decision Consumer Reconciliation; Evidence And Freeze Boundary Matrix;
Source Contradiction And Blocker Ledger; T2 Implementation Requirements;
Machine Closure Package; Epistemic Process; Agent Operation Trace; Public
Export Disposition; Claim Boundary.

Each continuation row must state producer, field/value, consumer, current
behavior, ratified doc-only disposition, required obligations/review, output or
provider reachability, freeze eligibility, source anchors, and contradiction.
...
workerReturnPath: `docs/reviews/CVF_SOT3_APP_T1_R1_WORKER_RETURN_2026-07-17.md`
...
| phase | `EXECUTION` |
| baseHeadFor(phase) | dispatchBaseHead=`3cf20ecce`; executionBaseHead=worker captures committed post-dispatch HEAD; closureBaseHead=reviewer sets |
...
| Field | Value |
|---|---|
| completionReviewPath | `docs/reviews/CVF_SOT3_APP_T1_R1_COMPLETION_REVIEW_2026-07-17.md` |
...
- Exact search commands reproduce both denominators and every returned file has one terminal inventory row.
- All eight local adapters, binding-health, and principal callers have direct-source compatibility rows.
- All five route decisions have an explicit disposition at every consumer.
- Kernel decisions remain a separate source-backed vocabulary.
- Packet identity remains separate from packet hash in both canonical T8 shapes.
- `ESCALATE` and `REVIEW_REQUIRED` fail closed to `HOLD_FOR_REVIEW` absent direct contrary authority.
- Current CVF public owners and the T8 owner API are cited from current source.
- Evidence and freeze authority ceilings are explicit.
- Every contradiction is terminally blocking or routed to one later tranche.
...
| Session or invocation | SOT3-APP-T1-R1 dispatch, 2026-07-17 |
...
| Before status evidence | clean worktree at `3cf20ecce` |
