...
| G4-GAP-04 | T3D may conclude only `SOURCE_CREATED_LOCAL_VERIFIED_PENDING_CONSUMER_BINDING` after its own bounded source checks. It must not check the T2F Group 4 real-verifier-consumer row. T3E exclusively owns the real verifier lookup call, final response-to-consumer comparison and final consumer-binding checklist disposition. | T3D pending-state positive; reject T3D `ESTABLISHED`, `CONSUMER_BOUND`, or final admission wording; require T3E-owned final row |
