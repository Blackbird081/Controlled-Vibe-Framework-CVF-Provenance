...
It also applies to changed work orders that report reusable lessons stored in
provider-specific memory. Provider memory may support the executing agent, but
it is not CVF source authority and cannot be the only durable location for a
reusable lesson.
...
agent autorun workflow, reviewer closeout, work-order authoring, and
operational session logs.
...
- a reusable lesson may be temporarily noted in provider memory, but must also
  be promoted or explicitly declined in a CVF-governed artifact;
- acceptable governed promotions are `RULE_ADDED`, `STANDARD_UPDATED`,
  `STANDARD_ADDED`, `MACHINE_CHECK_ADDED`, `MACHINE_CHECK_CANDIDATE`, or
  `TEMPLATE_UPDATED`;
- `N/A_WITH_REASON` is allowed only for non-reusable or session-local notes with
  a reason;
- "stored in memory" is not closure evidence for future-agent learning.
...
present without learning disposition. It also fails changed work orders that
state a reusable lesson was stored only in provider-specific memory without a
CVF-governed promotion disposition or explicit non-reusable/session-local
`N/A_WITH_REASON`.
...
- the disposition section lacks defect class, lane, disposition, or next
  action;
...
This is a governance-learning trigger standard. It makes findings actionable
and machine-visible; it does not prove that runtime learning, model learning,
