# CVF ASSF Package: Engineering Shipping And Launch
...
skillId: cvf-engineering-shipping-launch

## Purpose

Guide safe, observable, reversible production launch discipline: pre-launch checklists, staged rollout strategies, monitoring readiness, and rollback planning so every deployment is reversible, observable, and incremental. Use when a governed CVF task needs to deploy a feature to production, plan a release, migrate data or infrastructure, or open a beta program. Do not use when: runtime activation, automated production deployment execution without human oversight, provider/live proof, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-engineering-shipping-launch/` |
...
| Field | Value |
|---|---|
| Allowed task classes | launch-planning, release-preparation, rollout-design, rollback-planning |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-shipping-launch/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes automated production deployments, infrastructure changes, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/shipping-and-launch/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); pre-launch checklist discipline and staged rollout notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; production deployments, infrastructure changes, or beta program launches require separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if automated production deployment, infrastructure change, or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full pre-launch checklist and staged rollout guidance; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-shipping-launch/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-shipping-launch-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `shipping-and-launch` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not execute production deployments, modify infrastructure, implement a CLI/MCP adapter, or claim automatic invocation.
