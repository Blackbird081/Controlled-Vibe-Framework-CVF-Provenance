# CVF ASSF Package: Engineering Incremental Implementation
...
skillId: cvf-engineering-incremental-implementation

## Purpose

Guide thin-slice incremental delivery discipline: implement one piece, test it, verify it, then expand  -  leaving the system in a working state after each increment. Use when a governed CVF task needs to implement any multi-file feature or change where writing a large amount of code at once would risk untestable intermediate states. Do not use when: runtime activation, automated bulk implementation, provider/live proof, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-engineering-incremental-implementation/` |
...
| Field | Value |
|---|---|
| Allowed task classes | incremental-delivery, feature-implementation, multi-file-change, slice-authoring |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-incremental-implementation/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes automated bulk code generation, commit, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/incremental-implementation/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); thin-slice delivery discipline notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; bulk code generation, commit, or deployment requires separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if bulk automated implementation or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full thin-slice delivery process and increment validation guidance; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-incremental-implementation/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-incremental-implementation-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `incremental-implementation` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not execute automated bulk implementation, trigger commits, implement a CLI/MCP adapter, or claim automatic invocation.
