# CVF ASSF Package: Engineering Frontend UI
...
skillId: cvf-engineering-frontend-ui

## Purpose

Guide production-quality UI engineering discipline: accessible, performant, visually polished components that follow design system conventions and avoid generic AI-generated aesthetics. Use when a governed CVF task needs to build or modify user-facing interfaces, implement responsive layouts, manage UI state, or fix visual/UX issues. Do not use when: runtime activation, automated UI deployment, browser rendering proof, provider/live proof, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-engineering-frontend-ui/` |
...
| Field | Value |
|---|---|
| Allowed task classes | ui-component-design, frontend-implementation, layout-authoring, ux-improvement |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-frontend-ui/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes UI deployment, browser rendering, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/frontend-ui-engineering/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); UI engineering discipline notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; UI deployment, browser rendering, or production changes require separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if UI deployment, browser automation, or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full component architecture and UI engineering guidance; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-frontend-ui/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-frontend-ui-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `frontend-ui-engineering` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not deploy UIs, trigger browser rendering, implement a CLI/MCP adapter, or claim automatic invocation.
