# CVF ASSF Package: Engineering API And Interface Design
...
skillId: cvf-engineering-api-interface-design

## Purpose

Guide contract-first API, module, schema, and component-boundary design while preserving CVF source verification and deprecation discipline. Use when a governed CVF task needs to design new API endpoints, define module contracts, or establish interface boundaries. Do not use when: runtime activation, CLI/MCP execution, provider/live proof, production invocation, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-engineering-api-interface-design/` |
...
| Field | Value |
|---|---|
| Allowed task classes | api-design, interface-design, contract-review, implementation-planning |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-api-interface-design/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes commit, runtime, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/api-and-interface-design/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); risk trigger notes for public interface changes; recommended CVF owner-surface routing |
| Acceptance evidence | AGSK-R3 worker return with package-root proposal evidence; reviewer-fast gate PASS; anatomy checker PASS after PROPOSED promotion |
...
| Side effects | none from metadata reading; future use that edits files, calls APIs, or deploys requires separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF promotion/runtime tranche if active package instructions, tool execution, or authority above the active work order is needed |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-api-interface-design/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-api-interface-design-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `api-and-interface-design` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not activate the skill, authorize runtime execution, implement a CLI/MCP adapter, grant provider/live proof, or claim automatic invocation. Reviewer acceptance and UAT are required before APPROVED state; ACTIVE state requires a separate runtime tranche.
