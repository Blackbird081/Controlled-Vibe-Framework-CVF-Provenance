# CVF ASSF Package: Governance Documentation And ADRs
...
skillId: cvf-governance-documentation-adrs

## Purpose

Guide durable decision records and documentation discipline, capturing the why behind significant technical decisions using Architecture Decision Records (ADRs) and structured documentation practices. Use when a governed CVF task needs to record architectural decisions, document public API changes, onboard new agents, or capture context that future agents will need. Do not use when: runtime activation, automated documentation generation, provider/live proof, or authority beyond the active governed work order is required; also do not document obvious code or throwaway prototypes.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-governance-documentation-adrs/` |
...
| Field | Value |
|---|---|
| Allowed task classes | decision-recording, adr-authoring, documentation, onboarding |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-governance-documentation-adrs/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes automated doc generation, commit, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/documentation-and-adrs/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); ADR template and documentation discipline notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; automated documentation creation or commit requires separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if automated documentation generation or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full ADR template and documentation discipline guidance; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-governance-documentation-adrs/` |
...
| Invocation ID | `agsk-r3-package-cvf-governance-documentation-adrs-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `documentation-and-adrs` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not generate documentation automatically, implement a CLI/MCP adapter, or claim automatic invocation.
