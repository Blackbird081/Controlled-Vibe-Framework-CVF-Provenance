...
Guide decision records and durable documentation so future agents can understand why a governed change exists and how it should be used.

Use when a governed CVF task matches the `documentation-and-adrs` package pattern after explicit package selection and receipt-backed production execution. Do not use as autonomous runtime authority, provider routing authority, public-sync proof, merge approval, commit permission, or permission to bypass active CVF work-order gates.
...
| Field | Value |
|---|---|
| Allowed task classes | documentation-authoring, decision-recording, architecture-review |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-governance-documentation-adrs/, registry entries, truth packets, review artifacts |
| Risk ceiling | R0 |
...
| Trigger patterns | adr, architecture decision, documentation update, decision record, write docs |
...
| Invocation ID | `ascp-p4-p6-package-cvf-governance-documentation-adrs-2026-06-30` |
