# CVF ASSF Package: Engineering Performance Optimization
...
Guide measurement-first performance optimization discipline: profile before fixing, identify the actual bottleneck, fix it, measure again  -  never guess. Use when a governed CVF task has performance requirements in the spec, receives user/monitoring performance reports, shows below-threshold Core Web Vitals, or needs to build features handling large datasets or high traffic. Do not use when: runtime activation, live profiling execution, production performance testing without oversight, or authority beyond the active governed work order is required; also do not optimize before evidence of a problem exists.
...
| Field | Value |
|---|---|
| Allowed task classes | performance-analysis, bottleneck-identification, optimization-planning, profiling-review |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-performance-optimization/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes live profiling, production load tests, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/performance-optimization/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); measure-first optimization discipline notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; live profiling, load testing, or production optimization changes require separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if live profiling, production load testing, or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full profiling-first and optimization process guidance; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-performance-optimization/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-performance-optimization-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `performance-optimization` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not run live profiling, execute load tests, implement a CLI/MCP adapter, or claim automatic invocation.
