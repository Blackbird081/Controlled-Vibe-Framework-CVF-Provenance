# CVF ASSF Package: Governance Operator Interview
...
skillId: cvf-governance-operator-interview

## Purpose

Guide intent-extraction through one-question-at-a-time operator interviews, closing the gap between what operators ask for and what they actually need before any plan, spec, or code exists. Use when a governed CVF task request is underspecified (missing who, why, success criteria, or binding constraint), when the operator explicitly invokes clarification, or when silent assumption-filling would be cheaper to prevent now than to debug later. Do not use when: runtime activation, automated form-filling, provider/live proof, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-governance-operator-interview/` |
...
| Field | Value |
|---|---|
| Allowed task classes | intent-clarification, requirements-extraction, pre-plan-interview, ambiguity-resolution |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-governance-operator-interview/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes autonomous decision-making on behalf of the operator or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/interview-me/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); intent-extraction interview protocol notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; plan authoring or commitment requires separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if autonomous operator decision-making or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full interview protocol, 95% confidence calibration, and non-triviality criteria; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-governance-operator-interview/` |
...
| Invocation ID | `agsk-r3-package-cvf-governance-operator-interview-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `interview-me` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not autonomously make decisions on behalf of operators, implement a CLI/MCP adapter, or claim automatic invocation.
