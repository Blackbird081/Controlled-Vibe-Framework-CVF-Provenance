# CVF ASSF Package: Engineering Code Simplification
...
Guide behavior-preserving simplification of code that reduces complexity while maintaining exact behavioral contracts. Use when a governed CVF task needs to refactor working code for clarity, reduce nesting depth, consolidate duplicate logic, or extract focused helpers. Do not use when: runtime activation, automated refactoring execution, provider/live proof, or authority beyond the active governed work order is required; also do not use before understanding what the code does.
...
| Field | Value |
|---|---|
| Allowed task classes | code-refactoring, complexity-reduction, readability-improvement, dead-code-removal |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-code-simplification/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes autonomous refactoring, commit, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/code-simplification/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); simplification principle notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; automated refactoring or commit actions require separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if automated refactoring execution or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full simplification principles and process steps; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-code-simplification/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-code-simplification-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `code-simplification` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not execute automated refactoring, implement a CLI/MCP adapter, or claim automatic invocation.
