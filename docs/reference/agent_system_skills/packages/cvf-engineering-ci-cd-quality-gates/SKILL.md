...
Guide CI/CD quality gate design, pipeline evidence, and deployment automation planning without authorizing production deployment by package load.

Use when a governed CVF task matches the `ci-cd-and-automation` package pattern after explicit package selection and receipt-backed production execution. Do not use as autonomous runtime authority, provider routing authority, public-sync proof, merge approval, commit permission, or permission to bypass active CVF work-order gates.
...
| Field | Value |
|---|---|
| Allowed task classes | ci-cd-planning, quality-gate-design, release-readiness |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-ci-cd-quality-gates/, registry entries, truth packets, review artifacts |
| Risk ceiling | R1 |
...
| Trigger patterns | ci pipeline, quality gates, github actions, deployment pipeline, automation checks |
...
| Invocation ID | `ascp-p4-p6-package-cvf-engineering-ci-cd-quality-gates-2026-06-30` |
