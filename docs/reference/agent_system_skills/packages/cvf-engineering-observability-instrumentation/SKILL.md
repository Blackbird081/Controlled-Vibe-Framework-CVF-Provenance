# CVF ASSF Package: Engineering Observability And Instrumentation
...
skillId: cvf-engineering-observability-instrumentation

## Purpose

Guide observability-first development discipline: instrument code with logs, metrics, traces, and alerts alongside features so production behavior is visible and diagnosable from the outside. Use when a governed CVF task needs to add observability to a production feature, set up alerting, review a PR that adds I/O or cross-service calls, or respond to an incident caused by invisible system behavior. Do not use when: runtime activation, live provider telemetry injection, production monitoring deployment without oversight, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-engineering-observability-instrumentation/` |
...
| Field | Value |
|---|---|
| Allowed task classes | observability-design, instrumentation-authoring, alerting-setup, telemetry-review |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-observability-instrumentation/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes live telemetry injection, production monitoring deployment, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/observability-and-instrumentation/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); observability discipline and telemetry design notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; live telemetry deployment, alerting configuration, or production monitoring changes require separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if live telemetry deployment, production monitoring, or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full observability design and instrumentation guidance; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-observability-instrumentation/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-observability-instrumentation-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `observability-and-instrumentation` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not inject live telemetry, deploy monitoring configurations, implement a CLI/MCP adapter, or claim automatic invocation.
