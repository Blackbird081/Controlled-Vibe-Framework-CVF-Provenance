# CVF ASSF Package: Engineering Spec-Driven Development
...
Guide spec-first development discipline: write a structured specification before writing any code so requirements, acceptance criteria, and scope boundaries are explicit and agreed upon before implementation begins. Use when a governed CVF task starts a new project or feature, faces ambiguous or incomplete requirements, touches multiple files or modules, or needs more than 30 minutes to implement. Do not use when: runtime activation, automated spec generation without operator review, provider/live proof, or authority beyond the active governed work order is required.
...
| Does not apply to | `ACTIVE`, automatic resolver invocation, CLI/MCP adapter, provider/live proof, public-sync, implementation authority, commit authority, or production readiness |

## Invocation Boundary

| Field | Value |
|---|---|
| Allowed task classes | spec-authoring, requirements-definition, acceptance-criteria-design, scope-bounding |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-spec-driven-development/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes autonomous spec commitment, code generation, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/spec-driven-development/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); spec template and acceptance criteria discipline notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; spec commitment, code generation, or deployment requires separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if autonomous spec commitment, automated code generation, or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full spec-first process and structured specification template; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-spec-driven-development/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-spec-driven-development-2026-06-29` |
...
This package root is an APPROVED CVF adaptation sourced from the upstream `spec-driven-development` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It may be opened by the AGSK-R4 runtime package loader after AGSK-R7 lifecycle gates pass. It does not generate specs autonomously, commit implementation plans, implement a CLI/MCP adapter, or claim automatic invocation.
