# CVF ASSF Package: Governance Idea Refinement
...
skillId: cvf-governance-idea-refinement

## Purpose

Guide divergent-then-convergent idea shaping through structured expansion, stress-testing, and one-pager articulation, transforming vague concepts into concrete, actionable proposals worth building. Use when a governed CVF task starts with a raw or underspecified idea that needs variant generation, assumption surfacing, and convergence to a concrete plan. Do not use when: runtime activation, automated idea execution, provider/live proof, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-governance-idea-refinement/` |
...
| Field | Value |
|---|---|
| Allowed task classes | idea-exploration, concept-refinement, assumption-stress-test, proposal-authoring |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-governance-idea-refinement/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes autonomous idea execution, plan commitment, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/idea-refine/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); divergent-convergent refinement notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; plan commitment or build execution requires separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if automated plan execution or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full divergent/convergent refinement process and one-pager template; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-governance-idea-refinement/` |
...
| Invocation ID | `agsk-r3-package-cvf-governance-idea-refinement-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `idea-refine` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not execute plans autonomously, implement a CLI/MCP adapter, or claim automatic invocation.
