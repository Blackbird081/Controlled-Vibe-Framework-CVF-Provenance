# CVF ASSF Package: Engineering Source-Driven Development
...
skillId: cvf-engineering-source-driven-development

## Purpose

Guide official-documentation-backed implementation discipline: every framework-specific code decision must be grounded in current official docs, verified, and cited  -  preventing stale-training-data patterns from reaching production. Use when a governed CVF task builds with any framework or library where correctness and current best practices matter, or when the operator requests verified, source-cited code. Do not use when: runtime activation, automated documentation fetching without oversight, live web browsing for arbitrary pages, provider/live proof, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-engineering-source-driven-development/` |
...
| Field | Value |
|---|---|
| Allowed task classes | framework-implementation, source-verification, pattern-validation, documentation-grounding |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-engineering-source-driven-development/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes arbitrary web browsing, uncontrolled documentation fetching, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/source-driven-development/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); source-verification and citation discipline notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; web browsing, documentation fetching, or external requests require separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if arbitrary documentation fetching, uncontrolled web access, or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full official-doc verification and citation protocol; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-engineering-source-driven-development/` |
...
| Invocation ID | `agsk-r3-package-cvf-engineering-source-driven-development-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `source-driven-development` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not fetch arbitrary web documentation, implement a CLI/MCP adapter, or claim automatic invocation.
