# CVF ASSF Package: Governance Context Engineering
...
skillId: cvf-governance-context-engineering

## Purpose

Guide session and context setup discipline so agents receive the right information at the right time  -  preventing hallucination from missing context and focus loss from excessive context. Use when a governed CVF task requires starting a new session, improving agent output quality that has degraded, switching between codebase areas, or configuring context rules files. Do not use when: runtime activation, automatic context injection, provider/live proof, or authority beyond the active governed work order is required.

## Scope / Applies-To

| Field | Value |
|---|---|
| Package root | `docs/reference/agent_system_skills/packages/cvf-governance-context-engineering/` |
...
| Field | Value |
|---|---|
| Allowed task classes | session-setup, context-configuration, rules-file-authoring, quality-recovery |
| Allowed roles | dispatcher, worker, reviewer |
| Allowed phases | INTAKE, DISPATCH_AUTHORING, WORKER_EXECUTION, REVIEWER_CLOSURE |
| Allowed surfaces | docs/reference/agent_system_skills/packages/cvf-governance-context-engineering/, registry entries, review artifacts |
| Risk ceiling | R1 |
| Authority ceiling | metadata-only advisory; loading never authorizes automatic context injection, runtime session mutation, or external actions |

## Inputs And Outputs

| Field | Value |
|---|---|
| Inputs | operator request or governed task context; source mirror file `.private_reference/source_mirrors/addyosmani__agent-skills/skills/context-engineering/SKILL.md`; active CVF authority and allowed-scope boundaries |
| Outputs | proposed package-body guidance (awaiting reviewer acceptance); context hierarchy discipline notes; recommended CVF owner-surface routing |
...
| Side effects | none from metadata reading; automatic context injection or session-state mutation requires separate authorization |
| Rollback | delete this package root and revert registry entry to CANDIDATE; regenerate generated index |
| Safe stop | stop and open a fresh ASSF runtime tranche if automatic context injection, session mutation, or authority above the active work order is needed |
...
| Post-reviewer-acceptance (APPROVED) | full context hierarchy and rules-file authoring guidance; requires reviewer decision gate |
...
| Target paths | `docs/reference/agent_system_skills/packages/cvf-governance-context-engineering/` |
...
| Invocation ID | `agsk-r3-package-cvf-governance-context-engineering-2026-06-29` |
...
This package root is a PROPOSED CVF adaptation sourced from the upstream `context-engineering` skill at pinned commit `aba7c4e9695c363e65cb59effe926c7f1d1abe3d`. It does not inject context automatically, mutate session state, implement a CLI/MCP adapter, or claim automatic invocation.
