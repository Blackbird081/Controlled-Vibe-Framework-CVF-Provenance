...
| `RSPB-capability-preflight-bootstrap-runtime-and-checker-candidates` | RSPB-AI-T0 reverse-skill/Capability Preflight & Bootstrap dual-corpus intake | `RUNTIME_CANDIDATE` | A Capability Environment Snapshot (TTL-bound tool/dependency readiness evidence) and a controlled CHECK-PLAN-APPROVE-ACQUIRE-VERIFY acquisition/bootstrap transaction, adapted from upstream `reverse-skill` operational patterns and formalized in the local Capability Preflight & Bootstrap proposal's contracts, policies, and fixtures; a dependent repair-stop/integrity/secret-redaction `CHECKER_CANDIDATE` set only has value once the runtime candidate exists. | `T0_STOP_COST_EXCEEDS_VALUE_PARKED_UNTIL_CONDITION` | Independently decided `STOP_COST_EXCEEDS_VALUE` for the whole RSPB-AI-T0 tranche: every candidate in this row scored `PARKED_UNTIL_CONDITION` in the audit's Value/Cost Rubric because none has a named current non-test consumer or concrete blocked workflow. Reopen only when a fresh, source-verified runtime work order proves a concrete named current consumer and blocked workflow this snapshot/acquisition pattern would resolve, following the same evidentiary bar the `CADP-AI-external-readout-runtime-seam` row above applies. | `docs/audits/CVF_RSPB_AI_T0_DUAL_CORPUS_INTAKE_AUDIT_2026-08-15.md` | `EXTENSIONS/CVF_EXECUTION_PLANE_FOUNDATION/` runtime activation, `governance/compat/` checker wiring, dependency installation, external tool acquisition/execution, MCP/CLI transport activation, provider/live/network, credentials, public sync, deployment, production |
