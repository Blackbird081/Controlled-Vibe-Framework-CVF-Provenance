...
| `CADP-AI-downstream-consumer-adapters` | CADP-R1 F02/F06/F08 | `RUNTIME_CANDIDATE` | Internal execution-plane/model-gateway consumption and bounded SaaS constraint metadata without authority widening. | `T3A_ACCEPTED_BOUNDED_T3B_PARKED` | T3A independently accepts only the hermetic non-executing Execution Plane consumer. Reopen T3B only through a fresh Model Gateway source verification, governed work order, and operator authorization. | Guard Contract and Execution Plane accepted seam; Model Gateway remains parked | provider calls, credentials, CLI/MCP, SaaS execution, T3B |
