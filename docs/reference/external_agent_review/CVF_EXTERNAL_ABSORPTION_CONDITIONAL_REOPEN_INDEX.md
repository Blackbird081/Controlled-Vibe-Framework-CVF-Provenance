...
| `RSPB-capability-preflight-bootstrap-runtime-and-checker-candidates` | RSPB-AI-T0 reverse-skill/Capability Preflight & Bootstrap mixed-origin intake | `RUNTIME_CANDIDATE` | Capability Environment Snapshot is a high-value read-only candidate supported by current environment-blocker evidence; controlled acquisition remains a separate higher-risk conditional candidate; repair/integrity/secret/stale-state checkers follow selected contracts. | `CORRECTED_PROCEED_SELECTIVELY_RUNTIME_STAGED` | The former whole-tranche stop is superseded. Open the smallest read-only snapshot/profile owner-reconciliation tranche first. Keep mutating acquisition parked until a named workflow, mutation envelope, provenance/integrity plan, rollback, and receipt owner are approved. | `docs/assessments/CVF_RSPB_AI_T0_MIXED_ORIGIN_CORRECTIVE_REASSESSMENT_2026-08-16.md` | no direct import; no acquisition execution; no MCP/CLI activation; no provider/live/network, credentials, public sync, deployment, or production |
