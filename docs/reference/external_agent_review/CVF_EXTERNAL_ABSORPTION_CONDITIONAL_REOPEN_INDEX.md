...
- Reconciliation: manifest=7 seed-source artifacts; ledger_terminal=17 indexed candidate rows; exclusions=0; unresolved=0
- Unresolved files: 0
- Declared exclusions: none
- Unreadable or unsupported files: none
- Aggregation check: 17 indexed rows are sourced from 7 governed artifacts listed below.
...
| `CGE-code-intelligence-package-candidate` | CodeGraph CGE-R1 | `PACKAGE_CANDIDATE` | `cvf.code_intelligence` package shape could become a CVF-owned capability package after proof. | `PARKED_UNTIL_CONDITION` | Reopen after CGE-R2 proves useful value or operator authorizes a metadata-only ASSF package candidate tranche. | `docs/reference/CVF_CGE_R1_CODE_INTELLIGENCE_OWNER_SURFACE_MATRIX_2026-06-29.md` | ASSF package root, ACTIVE package state, resolver activation |
...
| CodeGraph package shape | Code-intelligence package candidate can enrich CVF package surfaces after proof. | `PACKAGE_CANDIDATE` | ASSF package candidate lane | CGE-R2 value probe or separate ASSF package work order | No package activation from this index |
| CodeGraph services and adapters | Graph-assisted context, impact, dependency, and staleness services may have practical CVF utility. | `RUNTIME_CANDIDATE` | Pending CGE-R2 value-probe roadmap | Open bounded value probe before any install or daemon work | No runtime activation from this index |
