...
| `RSPB-capability-preflight-bootstrap-runtime-and-checker-candidates` | RSPB-AI-T0 through T3 reverse-skill/Capability Preflight & Bootstrap mixed-origin intake | `IMPLEMENTED_BOUNDED_AND_CONDITIONAL_RUNTIME` | The read-only Capability Environment Snapshot is implemented in the existing doctor/pre-dispatch seam. The Controlled Acquisition Contract Kernel is implemented and runtime-tested in Guard Contract for canonical plan digest, exact approval binding, source/integrity/mutation-envelope checks, receipt reconciliation, and repair-stop decisions. | `PROCEED_SELECTIVELY_RUNTIME_STAGED_T3_KERNEL_ACCEPTED` | Knowledge and two selected runtime-value clusters are materialized. The contract kernel authorizes only a separately governed executor; it performs no acquisition. Reopen executor work only for a named workflow with approved mutation authority and evidence destination. | `docs/reviews/CVF_RSPB_AI_T3_CONTROLLED_ACQUISITION_CONTRACT_KERNEL_COMPLETION_2026-08-16.md` | no direct import; no acquisition execution; no MCP/CLI activation; no provider/live/network, credential access, public sync, deployment, or production |
