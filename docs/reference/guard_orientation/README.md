...
| Work-order authoring / dispatch | Dispatcher | `docs/reference/CVF_AGENT_WORK_ORDER_TEMPLATE_2026-05-19.md`; `docs/reference/CVF_TRANCHE_COMMIT_CHOREOGRAPHY_STANDARD_2026-06-03.md`; `docs/reference/CVF_DUAL_AGENT_SURFACE_ACCOUNTING_STANDARD_2026-06-23.md`; `docs/reference/review_cost_control/CVF_REVIEW_COST_AND_DIMINISHING_RETURN_CONTROL_STANDARD.md`; `AGENTS.md` Mandatory Work Order Source Verification section; applicable `governance/compat/check_*.py` sources | Source Verification Block (ACCEPT/REJECT/BLOCKED_SOURCE_NOT_FOUND per item); Checker Source Read-Ahead Block; Review Dispatch Convergence And Invocation Budget Control; Agent Handoff Contract Control Block; Dual Agent Surface Matrix with `INTERNAL_AGENT`, `EXTERNAL_AGENT_CLI_MCP`, and adapter boundary; Reviewer Closure Conversion; Commit Prompt Readiness | ACCEPT row cites no source file; work order dispatched without pre-dispatch gate; rework-by-drip without a consolidated finding digest; external re-dispatch without a parent cumulative ceiling; internal-only design omits external-agent disposition; using gate failures to discover required literal tokens | `python governance/compat/run_agent_autorun_workflow_gate.py --phase pre-dispatch` | Source facts require file and line/section before dispatch; round three cannot auto-dispatch; unknown external usage fails closed; direct out-of-band invocation remains outside this cooperative gate |
...
| Reviewer-return review | Reviewer | Work order; worker-return packet; GC-018 baseline; `docs/reference/review_cost_control/CVF_REVIEW_COST_AND_DIMINISHING_RETURN_CONTROL_STANDARD.md`; MFRP roadmap and active P4-C1 continuity; `docs/reference/CVF_DUAL_AGENT_SURFACE_ACCOUNTING_STANDARD_2026-06-23.md`; `docs/reference/agent_defect_intelligence/entries/CVF_ADIF-0026.md` | Reviewer decision; reviewer-fast pass; full contract/schema/path/authority/test/range/commit-plan matrix before first repair; review-cost and latency telemetry; boundary `EVALUATE_RETURNED_EVIDENCE_NOT_RECREATE_IMPLEMENTATION` | Accepting without reviewer-fast; sequential predictable findings; recreating orchestrator/worker work; per-row review; broad duplicate reruns without a named contradiction, expected information gain and cost reason; waiting for operator reminder to apply active measurement | `python governance/compat/run_agent_commit_steward_preflight.py --mode reviewer-return --base <closureBase> --head HEAD --enforce` | Consume valid returned/machine evidence; P4-C1 review only at M5/M10/safety/M20 or an admitted critical trigger; use the 10-minute bounded-local fast path when eligible; escalate at repair round three |
...
Convergence orientation: dispatchers generate INITIAL/REWORK control blocks
with `build_dispatch_packet_scaffold.py`; workers complete the generated
`Rework Convergence Self-Proof` before readiness; reviewers audit the whole
bounded dependency class before returning one consolidated repair. Keep
`successorTrancheOpened: NO` throughout that cycle. These controls govern
authority, evidence, budgets, invariants, and outcomes only; they must not
prescribe an agent's internal reasoning, algorithm, prompt, or tool order.
...
| Reviewer repeats predictable findings or spends about 30 minutes on a bounded local review | review-cost checker; ADIF-0026; completion-review scaffold | Complete the single-pass matrix before repair, run focused checks before broad gates, finish semantic repair before commit, default to one material plus one continuity commit, and record latency/avoidable-delay disposition |
...
| Warm workspace build masks a nested `file:` dependency failure | ADIF-0041 guidance; focused package-resolution regression | Before public push, reproduce from the exact public commit in a detached clean worktree, install only from the deployment base, then run TypeScript and the production build. Inspect sibling `node_modules`, `link: true` lock entries, and realpath search roots before changing dependency classes or enabling global symlink preservation. |
...
| Mixed-origin local synthesis is downgraded because it is unmerged, lacks one GitHub source, or sits outside current owner paths | `check_mixed_origin_derived_synthesis_absorption.py`; ADIF-0053 | Classify origin per concept; treat maturity separately from value; record independent knowledge/import/runtime/authority decisions; review the composed system chain |
...
| Field | Evidence |
|---|---|
| Actor | Codex orchestrator/reviewer and governance maintainer |
| Provider or surface | local private provenance repository |
| Session or invocation | P4-C1 universal reviewer-boundary discovery hardening, 2026-09-06 |
| Working directory | repository root |
| Command or tool surface | governed reads, `rg`, `apply_patch`, focused tests and autorun gates |
| Target paths | `AGENTS.md`; `docs/reference/CVF_AGENT_INSTRUCTION_CARRIER_ROUTING_INDEX_2026-08-11.md`; `docs/reference/guard_orientation/README.md`; `docs/reference/review_cost_control/CVF_REVIEW_COST_AND_DIMINISHING_RETURN_CONTROL_STANDARD.md`; `docs/roadmaps/CVF_DISPATCHER_ARCHITECTURE_READINESS_AND_QUOTA_ADMISSION_ROADMAP_2026-09-06.md`; `governance/compat/check_active_archive_hygiene.py`; `governance/compat/test_check_active_archive_hygiene.py`; `governance/compat/check_agent_instruction_carriers.py`; `governance/compat/test_check_agent_instruction_carriers.py` |
| Allowed scope source | operator instruction on 2026-09-06 to make active P4-C1 reviewer non-duplication and measurement universally known without reminders |
| Before status evidence | P4-C1 activation and reviewer boundary existed in continuity, MFRP and Review Cost surfaces but root `AGENTS.md` contained no MFRP/P4-C1 binding |
| After status evidence | root carrier exposes the invariant; routing and orientation point to existing owners; carrier checker/tests fail if mandatory tokens are removed |
| Diff evidence | exact nine-path material diff against `0bddbed4e`; two parked Phase-04 untracked files excluded and hash-preserved |
| Approval boundary | existing MFRP discoverability and carrier-integrity maintenance only |
| Claim boundary | no new reviewer workflow, DARA architecture gate, eligibility change, worker repair, external invocation, or runtime/provider/live/public action |
| Agent type | orchestrator/reviewer and governance maintainer |
| Invocation ID | `mfrp-p4-c1-universal-discovery-hardening-2026-09-06` |
| Expected manifest | `AGENTS.md`; `docs/reference/CVF_AGENT_INSTRUCTION_CARRIER_ROUTING_INDEX_2026-08-11.md`; `docs/reference/guard_orientation/README.md`; `docs/reference/review_cost_control/CVF_REVIEW_COST_AND_DIMINISHING_RETURN_CONTROL_STANDARD.md`; `docs/roadmaps/CVF_DISPATCHER_ARCHITECTURE_READINESS_AND_QUOTA_ADMISSION_ROADMAP_2026-09-06.md`; `governance/compat/check_active_archive_hygiene.py`; `governance/compat/test_check_active_archive_hygiene.py`; `governance/compat/check_agent_instruction_carriers.py`; `governance/compat/test_check_agent_instruction_carriers.py` |
| Actual changed set | `AGENTS.md`; `docs/reference/CVF_AGENT_INSTRUCTION_CARRIER_ROUTING_INDEX_2026-08-11.md`; `docs/reference/guard_orientation/README.md`; `docs/reference/review_cost_control/CVF_REVIEW_COST_AND_DIMINISHING_RETURN_CONTROL_STANDARD.md`; `docs/roadmaps/CVF_DISPATCHER_ARCHITECTURE_READINESS_AND_QUOTA_ADMISSION_ROADMAP_2026-09-06.md`; `governance/compat/check_active_archive_hygiene.py`; `governance/compat/test_check_active_archive_hygiene.py`; `governance/compat/check_agent_instruction_carriers.py`; `governance/compat/test_check_agent_instruction_carriers.py` |
| Manifest delta | MATCH |
| Deletion or rename disposition | N/A with reason: none |

## Historical Agent Operation Trace Block - 2026-07-12
