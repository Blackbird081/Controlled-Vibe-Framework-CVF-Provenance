...
| F06 SaaS/remote side-effect discipline | credentials by reference, quota/cost/rate/retention and side-effect limits | external capability admission + gateway | T3A_EXECUTION_CONSUMER_ACCEPTED_BOUNDED; T3B_MODEL_GATEWAY_ACCEPTED_BOUNDED_NON_EXECUTING |
...
| T5 | optional external-agent adapter decision | CLI/MCP read/query interface or explicit rejection | accepted T3/T4 plus operator authorization | auth, ingress, mutation, redaction and dual-surface proof | DISPATCH_READY_DECISION_ONLY |
