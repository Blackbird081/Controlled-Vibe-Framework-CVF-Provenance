...
Status: T0_T1_ACCEPTED_BOUNDED_T2A_DISPATCH_READY
...
| F05 work-order capability binding | exact grant, resource, transport, expiry, invocation/retry reconciliation | existing workflow/work-order owners | T2_PLANNED |
| F06 SaaS/remote side-effect discipline | credentials by reference, quota/cost/rate/retention and side-effect limits | external capability admission + gateway | T3_PLANNED_NO_LIVE_PROOF |
| F07 direct import/authority | raw project, generated authority, duplicated owner | none | TERMINAL_REJECT |
| F08 local validator/projection/reconciliation | pure fail-closed validation first, projections remain non-authoritative | Guard Contract then consumer adapters | PARTIAL_T1; T2/T3_PLANNED |
| F09 negative-case suite | adversarial T1 validator/receipt tests plus future cross-owner fixtures | owning package test suites | T1_NEGATIVE_SUITE_ACCEPTED_BOUNDED; T2/T3_PLANNED |
...
| F11 evidence authenticity gap | reject opaque refs unless type, integrity, authority and owner match within an owner-verified evidence projection | Guard Contract evidence index | F11_RESIDUAL_OPEN_CALLER_SELF_ATTESTATION; T2 owner binding released |
...
Reviewer/closer commits the accepted T0/T1 material, then dispatches a fresh
source-verified T2 owner-binding work order under the operator's explicit
2026-08-13 release. T3-T7 are not implicitly authorized.
...
This roadmap records bounded T0/T1 acceptance and operator release to author
and dispatch T2. F11 remains open until T2 is independently accepted. No
runtime integration, trusted-evidence, provider compatibility, CLI/MCP, SaaS
execution, deployment, production readiness, or public export is claimed.
