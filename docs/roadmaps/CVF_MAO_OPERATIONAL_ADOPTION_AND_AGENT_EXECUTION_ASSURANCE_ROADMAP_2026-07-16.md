...
- route: one `WORKER_MUST_NOT_COMMIT` direct-call calibration followed by
  independent reviewer/closer and separate session-sync steward;
- boundary: one Alibaba/DashScope-compatible call, zero retries, secret-safe
  evidence, and no MAO comparison in T6A; and
