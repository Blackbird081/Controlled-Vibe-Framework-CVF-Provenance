...
| T0A | resolve identity, execution-port/provider-boundary, routing/quota/credential, attempt-admission and rollback ownership | exact owner/interface decision and corrected smallest T1 manifest | `CLOSED_PASS_BOUNDED`; terminal `READY_FOR_T1_CANONICAL_EXECUTION_PORT_DESIGN` |
| T1 | freeze canonical execution envelope, execution port, correlation and compatibility contract | contract/reference decision, adapter boundary, identity/receipt invariants, legacy-path disposition, deterministic test plan | `CLOSED_PASS_BOUNDED`; terminal `READY_FOR_T2_CANONICAL_WEB_GATEWAY_COMPOSITION` |
| T2 | compose Web text execution through Model Gateway | bounded source adapter/cutover, no duplicate provider call, parity and fail-closed deterministic tests, reversible rollback boundary | `CLOSED_BLOCKED_BOUNDED`; terminal `PARTIAL_IMPLEMENTATION_CONTRACT_OR_MANIFEST_CONFLICT`; additive port/identity foundation retained dormant, no Web route cutover |
| T2A | reconcile route-build selection and production provider-parity ownership | decide route-wide versus explicitly bounded provider migration; inventory protocol-compatible Gateway adapters, credential/config owners and exact successor implementation manifest | `DISPATCH_READY`; work order `docs/work_orders/CVF_AGENT_WORK_ORDER_CSCC_R1_T2A_ROUTE_SELECTION_AND_ALIBABA_PROVIDER_PARITY_RECONCILIATION_2026-09-03.md`; documentation/source-reconciliation only; operator-granted Alibaba live proof remains reserved and is not consumable in T2A |
| T3 | complete correlated receipt and durable lineage | one execution identity joins Guard, SOT3 reference, Gateway receipt/manifest, provider-attempt, validation, audit and readout across terminal paths | `HOLD_T2A_AND_ACCEPTED_CANONICAL_ROUTE` |
...
T1 dispatch authoring after independent T0/T0A acceptance.
