...
| T2 | compose Web text execution through Model Gateway | bounded source adapter/cutover, no duplicate provider call, parity and fail-closed deterministic tests, reversible rollback boundary | `READY_FOR_DISPATCH_AUTHORING`; implementation remains held until a fresh governed T2 packet passes dispatch gates |
