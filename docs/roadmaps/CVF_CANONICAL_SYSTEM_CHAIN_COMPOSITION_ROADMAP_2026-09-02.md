...
| T2A | reconcile route-build selection and production provider-parity ownership | decide route-wide versus explicitly bounded provider migration; inventory protocol-compatible Gateway adapters, credential/config owners and exact successor implementation manifest | `CLOSED_PASS_BOUNDED`; terminal `STOP_NO_SAFE_CANONICAL_CUTOVER`; composition, credential, quota and rollback-test ownership are reconciled, but no accepted route-build exclusion/packaging mechanism exists; no T2B or live proof opened |
| T3 | complete correlated receipt and durable lineage | one execution identity joins Guard, SOT3 reference, Gateway receipt/manifest, provider-attempt, validation, audit and readout across terminal paths | `HOLD_STOP_NO_SAFE_CANONICAL_CUTOVER` |
