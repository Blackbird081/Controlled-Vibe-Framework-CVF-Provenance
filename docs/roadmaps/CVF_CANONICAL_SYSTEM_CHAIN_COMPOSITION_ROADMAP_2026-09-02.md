...
| T2A | reconcile route-build selection and production provider-parity ownership | decide route-wide versus explicitly bounded provider migration; inventory protocol-compatible Gateway adapters, credential/config owners and exact successor implementation manifest | `READY_FOR_DISPATCH_AUTHORING`; documentation/source-reconciliation only; no provider/live call or route cutover |
