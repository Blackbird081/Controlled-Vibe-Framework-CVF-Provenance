...
| T1 | freeze canonical execution envelope, execution port, correlation and compatibility contract | contract/reference decision, adapter boundary, identity/receipt invariants, legacy-path disposition, deterministic test plan | `READY_FOR_DISPATCH_AUTHORING`; documentation-only integrated root contract |
