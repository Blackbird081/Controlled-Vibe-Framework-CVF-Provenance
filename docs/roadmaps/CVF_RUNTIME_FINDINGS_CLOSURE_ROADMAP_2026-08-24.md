...
| R7A | R6 audit accepted; operator approved recommended split | make all MCP production roots adopt canonical `cvf-guard-contract`; preserve MCP-local session phase without a replacement factory | `DISPATCH_READY_PENDING_MATERIAL_COMMIT`: Defect A only |
| R7B | R7A independently accepted | repair optional-field composition seam while preserving strict fail-closed material-context validation | `HOLD_DEPENDENCY_GATED`: no worker dispatch until R7A closure evidence is committed |
...
The operator approved the recommended split. R7A is freshly source-verified
and dispatch-ready pending its material packet commit; R7B remains held until
R7A is independently accepted. No source edit or external effect is released
by this roadmap transition.
