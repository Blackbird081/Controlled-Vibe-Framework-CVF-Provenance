...
Status: SOT3_APP_T1_R2_DISPATCHED_WORKER_NEXT
...
| Control | Decision |
|---|---|
| current active tranche | T1-R2 exact inventory membership and caller-closure documentation worker execution |
| committed packet | prior paired T0 GC-018 and work order are held and must not be executed |
| scheduling release | T1-R2 worker only; T2 and later remain parked until accepted contract-inventory closure |
...
| Roadmap state | this artifact | `Status: SOT3_APP_T1_R2_DISPATCHED_WORKER_NEXT` | PASS |
...
| System loop interlock | T0B -> MAO-OA -> T1 -> T1-R1 review -> T1-R2 execution/review | T2 and later remain parked until accepted contract-inventory closure | PASS |
