# GC-018 Authorization — W108-T1: AI Modals Visual Sync
...
Restyle the three AI modal components to match dark theme design tokens established in W105-T1 and W106-T1.
...
| `src/components/MultiAgentPanel.tsx` | 659 | CSS-only (advisory: > 600 — no new lines added) |
...
- Exported component signatures must remain frozen: `AgentChatWithHistory({ initialPrompt?, onClose?, onMinimize?, onComplete? })`, `MultiAgentPanel({ initialInput?, onComplete?, onClose? })`, `ToolsPage({ onClose? })`
...
- MultiAgentButton and ToolsButton exported helpers: CSS-only updates permitted (gradient → solid indigo/amber unchanged)
...
| Token | Value |
| --- | --- |
| Background | `#0d0f1a` → `dark:bg-[#0d0f1a]` |
...
- CP2 (Tier A): `tsc --noEmit` + `npm run lint` + `npm run build`
