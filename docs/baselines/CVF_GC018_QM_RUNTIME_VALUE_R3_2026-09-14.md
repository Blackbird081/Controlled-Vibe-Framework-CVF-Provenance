# CVF GC-018 Baseline - QM Runtime Value R3
...
accounts for runtime/use-case value in QM's `src/acl`, `src/classify`, `src/policy`,
and `src/security` trees at the indexed immutable pin.
...
1. Exact mirror HEAD equals `59cf6554faadcd06494782190c3ecae1829dd381`,
...
- Corpus root: `src/acl`, `src/classify`, `src/policy`, `src/security` at the required QM pin.
...
- Reconciliation: dispatch state `manifest=8; ledger_terminal=0; exclusions=0; unresolved=8`; accepted return must reach `manifest=8; ledger_terminal=8; exclusions=0; unresolved=0`.
- Declared exclusions: other QM regions; linked tests use a separate ledger.
- Unresolved files: 8 at dispatch; worker must account for every row and reach unresolved=0.
...
## R1/R2 Dependency And R3 Boundary

Accepted R1 completion: `docs/reviews/CVF_QM_RUNTIME_VALUE_R1_COMPLETION_2026-09-14.md`
at `c3b38dffda7f00778803d2c976926064d8f86b15`.
Accepted R2 completion: `docs/reviews/CVF_QM_RUNTIME_VALUE_R2_COMPLETION_2026-09-14.md`
at `e081c2eb6e2dfbb218e646609258d04fa3e1f8d4`.
R3 is an independent INITIAL lane inside the existing program.
Its eight target blobs (acl=3, classify=1, policy=1, security=3) have no target-path overlap with R1/R2.
Linked prior tests require exact-blob and full-read provenance before evidence reuse.
Dispatcher manifest SHA-256: `6be5746434e9a591b0b57be6a453bb5466ea2b460a5327e673ce9a81f0f87d50`.
The selected upstream pin is `59cf6554faadcd06494782190c3ecae1829dd381`;
the paired work order records the live freshness receipt and digest recipe.
No full-QM claim or source implementation is authorized.
