# CVF GC-018 Baseline - QM Runtime Value R2
...
accounts for runtime/use-case value in QM's `src/runs`, `src/sessions`,
and `src/processes` trees at the indexed immutable pin.
...
2. A deterministic manifest covers every blob under the three target trees.
...
| Enumeration or manifest plan | exact-pin `git ls-tree` for three named trees plus linked-test ledger |
...
- Corpus root: `src/runs`, `src/sessions`, `src/processes` at the required QM pin.
...
- Reconciliation: dispatch state `manifest=27; ledger_terminal=0; exclusions=0; unresolved=27`; accepted return must reach `manifest=27; ledger_terminal=27; exclusions=0; unresolved=0`.
- Declared exclusions: other QM regions; linked tests use a separate ledger.
- Unresolved files: 27 at dispatch; worker must account for every row and reach unresolved=0.
...
This baseline authorizes a static, evidence-producing audit of three QM source
...
## R1 Dependency And R2 Boundary

Accepted R1 completion: `docs/reviews/CVF_QM_RUNTIME_VALUE_R1_COMPLETION_2026-09-14.md`
at `c3b38dffda7f00778803d2c976926064d8f86b15`. R2 is an independent INITIAL lane
inside the existing program, not another repair round or stopped-chain successor.
The 27 target blobs (runs=18, sessions=6, processes=3) have no target-path overlap
with R1. Linked R1 tests may be reused with exact-pin/full-read provenance.
Dispatcher manifest SHA-256: `9fab809cb263c7cc38ff707a1bc794c692730ebbc918767821919a3dd0a21709` (recipe and worker contract in paired work order).
