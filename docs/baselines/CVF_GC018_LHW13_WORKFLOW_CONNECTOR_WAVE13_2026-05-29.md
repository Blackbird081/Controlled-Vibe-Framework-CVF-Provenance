...
| `MemoryGatewayDecision.canReinject` | `EXTENSIONS/CVF_LEARNING_PLANE_FOUNDATION/src/controlled-memory-gateway.ts` | line 49 — `canReinject=false` preserved |
...
| AIF-B graph modules | `EXTENSIONS/CVF_LEARNING_PLANE_FOUNDATION/src/graph-schema.ts` | `GraphKnowledgeService` interface |
