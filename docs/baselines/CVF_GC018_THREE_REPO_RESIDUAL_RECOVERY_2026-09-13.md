# CVF GC-018 - Three-Repository Residual Recovery And Program Continuation
...
Release docs/work_orders/CVF_AGENT_WORK_ORDER_THREE_REPO_RESIDUAL_RECOVERY_2026-09-13.md for bounded residual evidence recovery across the current three
repositories and preservation of the larger program backlog. Operator explicitly
requested this work order for Claude and autonomous Local continuation.

## Decision / Baseline

APPROVED_FOR_EXECUTION after required pre-dispatch PASS. Exact two-output worker
scope only; WORKER_MUST_NOT_COMMIT. Initial survey-stage evidence recovery does
not accept absorption or implementation. Current batch is not the full program.
...
Worker creates docs/audits/CVF_THREE_REPO_RESIDUAL_RECOVERY_2026-09-13.json and docs/reviews/CVF_THREE_REPO_RESIDUAL_RECOVERY_WORKER_RETURN_2026-09-13.md only. Three frozen mirror pins and read-only
metadata/backlog discovery are defined in the packet. No mirror/INDEX mutation,
network acquisition or source execution. Local also owns docs/reviews/CVF_THREE_REPO_RESIDUAL_RECOVERY_COMPLETION_2026-09-13.md at return. Local owns this pair and the exact
seven continuity paths above. Active worker lane forbids concurrent mutation.

## Source Verification Block

| Fact | Source file | Verified section | Disposition |
|---|---|---|---|
| Residual obligations | docs/reviews/CVF_THREE_REPO_ABSORPTION_SCOPE_RECOVERY_2026-09-13.md | Three-Repository Obligation Matrix | ACCEPT |
| Current batch is part of umbrella program | docs/reference/external_agent_review/CVF_CROSS_WORKSPACE_DOMAIN_FUNNEL_ABSORPTION_METHOD.md | Pilot And Scale-Out Admission | ACCEPT |
| Reported wider source seed | AGENT_HANDOFF_V60_2026-09-08.md | Latest Work / Changes | ACCEPT |

Reported seed totals are unverified intake, not Local corpus-completeness proof.

## Acceptance Criteria

Paired work-order criteria control: all three sources accounted with honest
read depth and maturity; external/Local reconciliation; broader backlog retained;
concrete next action; two owned outputs and mandatory gates; no source acceptance.

## Evidence / Verification

At 6d9e8248fbbf044054b2e1942aef49e9c0a8e5df, worktree was clean and all three local mirror HEADs matched the
packet's exact pins with clean statuses. Source residual reading is not performed
by dispatch author. Pre-dispatch and normal staged pre-commit gate the new pair;
material and dedicated continuity ranges receive separate pre-closure receipts.
The packet routing manifest is survey-stage EXTERNAL_ABSORPTION with externalEffect
NONE; existing intake receipt is reused, selected residual reads remain pending.

## Non-Goals And Stop Conditions

No source import, implementation, acquisition/fetch, upstream execution, provider,
credentials, public/push/deploy. Source-local gaps remain visible while independent
work continues. Budget exhaustion yields partial evidence and a next action.

## Dual Agent Surface Matrix

| Consumer class | Interface or owner surface | Authority and risk boundary | Evidence | Adapter boundary | Disposition |
|---|---|---|---|---|---|
| INTERNAL_AGENT | Paired intake work order | Released bounded same-workspace survey; no worker commit | Operator preparation request | Document contract only | CONTRACT_ONLY |
...
| Session or invocation | three-repo-recovery-dispatch-20260913 |
| Working directory | repository root |
| Command or tool surface | Read-only Git/filesystem; scaffold helper; document authoring and gates |
| Target paths | This baseline and paired work order |
| Allowed scope source | Operator instruction to record continuing multi-repo program and issue Claude packet |
| Before status evidence | Clean at 6d9e8248fbbf044054b2e1942aef49e9c0a8e5df |
| After status evidence | Two dispatch documents; continuity follows separately |
| Diff evidence | git status and exact pending files |
| Approval boundary | Bounded internal evidence recovery |
| Claim boundary | No source acceptance or implementation |
...
SKIPPED_WITH_REASON: baseline preparation only; paired work order defines the exact source identities, mirror plan, ledger, overlap and value routes for read-only recovery; no acquisition authority.
