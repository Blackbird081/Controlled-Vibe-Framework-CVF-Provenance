# CVF GC-018 Baseline - MFRP-P3-R1A Committed Oracle Ratification
...
Authorize one provider-free documentary and machine-data pass that authors a
source-bound actual-seam replay oracle and a ratification return for independent
review. R1A performs static feasibility analysis only. It does not execute a
replay case, create a runner, observe a runtime result, modify P2, or open R1B.
...
|---|---|
| operator instruction on 2026-09-01 | authoring of a fresh R1A baseline/work order is authorized; execution remains separately governed |
...
R1A creates exactly two new paths: one JSON oracle and one full ratification
return. The JSON is normative input for a possible later R1B, not observed
evidence. The worker may inspect current sources and calculate hashes/excerpt
digests but must not invoke the receipt validator or readout builder on replay
cases, create a receipt, add a helper/test, or claim an observed outcome.
...
|---|---|
| `governance/compat/fixtures/mfrp_p3_r1a_actual_seam_replay_oracle.json` | CREATE one canonical JSON oracle; no executable code |
| `docs/reviews/CVF_MFRP_P3_R1A_COMMITTED_ORACLE_RATIFICATION_WORKER_RETURN_2026-09-01.md` | CREATE source, feasibility, identity, digest and no-execution evidence return |
...
- need to invoke P2 seams, build receipts, run replay or create helper/tests;
...
- Baseline, work order and both worker-output paths were absent before
  authoring.
- Exact searches covered `MFRP-P3-R1A`, `actualSeamReplayOracle` and the four
