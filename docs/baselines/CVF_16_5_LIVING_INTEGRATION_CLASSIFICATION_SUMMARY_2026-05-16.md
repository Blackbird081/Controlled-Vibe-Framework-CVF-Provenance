...
| Controlled Memory | `agentmemory`, OpenAgentd memory notes | High | `docs-classified` | Learning Plane + Context Builder + Guard Contract | memory capture/retrieval/reinjection goes through privacy filter, lifecycle policy, access policy, context packager, and receipt |
...
2. **Controlled Memory or Agent/Tool Boundary**

Reason: high-fit governance value and direct continuity with CVF's non-coder and
multi-agent posture.

Decision needed after Model Gateway: choose one, not both in the same runtime
tranche.

3. **MCP Business Adapter**
...
4. **Observability Delta**

Reason: useful, but must be a delta on v1.8.1 rather than a new plane.

5. **Artifact Renderer / OpenSpec / Skill Evolution / Knowledge Vault**
...
| Controlled Memory | GC-018, privacy/access policy, memory lifecycle schema, context reinjection proof plan |
...
Model Gateway is the only CVF 16.5 lane currently at `roadmap-ready`. All other
lanes remain `docs-classified` until a new roadmap-ready packet is prepared.
