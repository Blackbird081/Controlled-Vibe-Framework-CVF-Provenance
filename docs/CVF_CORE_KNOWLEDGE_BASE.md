...
| CVF_GC051_CORPUS_SCAN_REGISTRY_GUARD.md | Corpus scan, classification, or knowledge absorption intent | Consult and update `docs/corpus-intelligence/CVF_CORPUS_SCAN_REGISTRY.json` before re-scan; preserve machine-readable scan state and drift evidence |
...
Governed files must be kept reviewable from project start, not merely trimmed
at the end to satisfy line-count checks.

Canonical rule:

- agents must plan rotation, archive, split, or refactor actions when active
  session, handoff, review, work-order, policy, or roadmap files approach hard
  thresholds
- touched near-threshold governed files must either gain a same-domain
  rotation/split artifact or be materially reduced in size before closure
- last-minute wording compression that preserves an oversized ownership surface
  is a maintainability defect
