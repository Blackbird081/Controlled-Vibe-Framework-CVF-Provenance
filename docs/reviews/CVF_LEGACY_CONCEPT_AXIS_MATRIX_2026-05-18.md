...
| Per-agent audit receipt | `.private_reference/legacy/CVF 16.5/Claude Kit/CVF_AGENT_AUDIT_RECEIPT.md`; `.private_reference/legacy/CVF 16.5/pancake-pos-mcp/Thong_tin.md` | `EXTENSIONS/CVF_GUARD_CONTRACT/src/contracts/receipt-envelope.contract.ts`; `EXTENSIONS/CVF_GUARD_CONTRACT/src/contracts/receipt-binding.contract.ts`; `EXTENSIONS/CVF_v1.6_AGENT_PLATFORM/cvf-web/src/lib/workflows/workflow-resolver.ts` | `partially_absorbed` | medium | Phase E E.5 selected-flow receipt binding covers active Product Brief role/action pairs only; full `CVFRole x ToolActionClass` matrix remains deferred_with_reason | none |
