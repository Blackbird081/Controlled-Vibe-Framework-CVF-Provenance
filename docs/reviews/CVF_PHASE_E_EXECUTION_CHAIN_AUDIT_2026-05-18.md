...
| CVF role resolution | `session?.role` raw string used in envelope, guard context, and audit payloads | `role-permission.contract.ts` | relevant | wiring_gap | E.2 adds `resolveExecutionCVFRole()` and emits normalized `CVFRole` trace/result before provider dispatch. |
| Output class gate | no pre-dispatch output-class permission gate | `RolePermissionOutputClass` / `isOutputAllowedForRole()` | relevant | wiring_gap | E.2 adds `resolveExecutionOutputClass()` and checks `isOutputAllowedForRole()` before provider dispatch. |
...
| Workflow transition | no binding lookup or `WorkflowTransition` trace in `route.ts` | `runtime-workflow.contract.ts` | relevant | wiring_gap | E.3 defines binding; E.4 resolves the binding and emits ordered `WorkflowStepExecutionTrace` records. |
| Provider call | `routeWebProvider()` then `executeAI()` | `ToolActionClass.provider_call` | relevant | wiring_gap | E.4 types the provider call as an active workflow step and records its trace in response and audit event. |
| Output validation | `validateOutput()` is called after provider response/retry | role output class plus validator | relevant | wiring_gap | E.4 ties validation result to the workflow step trace and E.2 ensures role/output-class permission already passed before dispatch. |
...
| Governance receipt | `buildEvidenceReceipt()` and `buildGovernanceEnvelope()` used in multiple response paths | workflow receipt obligation / receipt binding | relevant | wiring_gap | E.4/E.5 tie receipt IDs to receipt-required workflow steps and include them in `stepTraces[]` and audit event payloads. |
| Audit event | `appendAuditEvent()` called for block, guard, bypass, and completion paths | audit evidence surface plus Phase D role/action context | relevant | wiring_gap | E.4 audit event must include role permission result and `stepTraces[]` for the bound flow. |
| Response | governed response includes result, evidence receipt, envelope, and Phase 2.C/3.E fields | workflow response trace obligation | relevant | wiring_gap | E.4 response includes `workflowId`, `stepTraces[]`, and receipts for the bound flow without breaking unbound fallback responses. |
