...
| EVT4-02 | Launch options memo | OK | 1.00 | 0.68 | -0.32 |
...
| EVT4-08 | Ops plan | OK | 0.88 | 0.72 | -0.16 |
...
| EVT4-18 | Freemium decision | OK | 0.96 | 0.80 | -0.16 |
...
The retained runtime change is the template-specific task-shape override after
the rendered trusted-form skeleton. It reduced the corrected-template quality
gap but did not meet parity: median `CFG-B - CFG-A` remains `-0.16`, below the
registered `>= -0.05` decision rule.

This run supports keeping bounded shape guidance for non-coder plan,
comparison, FAQ, prioritization, persona, and acceptance-criteria requests. It
does not support claiming EVT-4 quality closure. The remaining gap should be
handled by template-specific redesign or new trusted template families, not by
more broad global prompt instructions.
