...
Current closure anchor: `03f5f3ce` (feat(int1): Generic MCP Adapter). Parent governed closure anchor: `b4be3895` (feat(cbp1): Context Budget Policy + governance pre-existing violation fix). Derive the current HEAD live with `git rev-parse --short HEAD` when needed.
