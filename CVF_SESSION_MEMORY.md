...
Mode: `gclh_mfrp_p4_c1_automatic_evidence_collection_active`. Accepted P4 material is `b9bdba712`; eligible count remains 0 and the M5/M10/safety/M20 rules are unchanged. DARA-T2B internal recovery dispatch is committed at `385aa5f74` under `OPERATOR_AUTHORIZED_INTERNAL_RECOVERY`. Only a qualifying `INTERNAL_AGENT` may execute; the operator manually relays the packet, the orchestrator/reviewer makes no external-worker CLI call, external invocation count remains 2/2, and the worker may edit only the exact 14-path candidate without commit. Manual relay across an independent provider/account boundary is external and must stop as `BLOCKED_SURFACE_MISMATCH`. DARA-T3 and `WP-ARCH-003` remain parked. In the independent System Chain lane, CSCC-R1-T2A remains stopped at `c1f95ae1d`; provider/live/public and other parked lanes remain closed. Latest finalized numbered LHW wave remains LHW24.
