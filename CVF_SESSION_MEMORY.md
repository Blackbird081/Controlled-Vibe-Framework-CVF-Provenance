...
Current mode marker: `rspb_ai_t14_capability_preflight_advisory_operator_projection_kernel_dispatched_pending_worker`
Current mode: `rspb_ai_t14_capability_preflight_advisory_operator_projection_kernel_dispatched_pending_worker`
Previous mode: `rspb_ai_t13_closed_bounded_next_absorption_tranche_selection`

## Next Allowed Move

Mode: `rspb_ai_t14_capability_preflight_advisory_operator_projection_kernel_dispatched_pending_worker`; dispatch `ccc15d893`; operator transfers the exact-five-path no-commit packet, then independent reviewer verifies; TPGR selective execution remains forbidden.
...
RSPB-AI-T14 is dispatched at `ccc15d89345af977656dc66b0ab8cf274d0a0b92`: seven advisory-projection source hashes matched and all seven files were fully read; direct import was rejected because the proposal uses coercive mapping, React callbacks, and action semantics. Pre-dispatch passed 78/78 and pre-commit passed 86/86. Worker scope is exactly five unstaged/uncommitted paths with `COMPLETE_PENDING_REVIEW` or `BLOCKED_WITH_REASON`; independent review owns all acceptance, repair, commit, and closure. No UI/API/callback/approval/action execution, registry/generated-index change, package loading, runtime, provider/live, public, deploy, or production authority is opened.
