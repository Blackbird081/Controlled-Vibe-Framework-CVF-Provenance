#!/usr/bin/env python3
"""Check external provider skill usage trace blocks in changed CVF artifacts.

The checker is intentionally narrow. It does not inspect provider-owned
runtime behavior. It only requires a trace block when a changed governed
artifact claims that an agent used, invoked, called, ran, or executed an
external/provider-owned skill and consumed that result in CVF.
"""
...
TRACE_HEADING = "## External Provider Skill Usage Trace"
NEXT_HEADING_RE = re.compile(r"^##\s+", re.MULTILINE)

USAGE_PATTERNS = (
    re.compile(
        r"\b(?:used|invoked|called|ran|executed)\b.{0,80}"
        r"\b(?:external\s+provider\s+skill|provider-owned\s+skill|"
        r"provider\s+skill|codex\s+skill|claude\s+skill|gemini\s+skill|"
        r"\$?[a-z][a-z0-9_-]{2,}\s+skill)\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:providerSkillUsed|externalProviderSkillUsed)\s*[:=]\s*"
        r"(?:true|yes|USED_WITH_TRACE)\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:Provider skill used|External provider skill used|Skill used)\s*:\s*\S+",
        re.IGNORECASE,
    ),
)
...
REQUIRED_ROWS = (
    ...
    "CVF source-of-truth promotion path",
    "Evidence artifact",
    "Authority boundary",
)

USED_TOKEN = "USED_WITH_TRACE"
...
def has_provider_skill_usage_claim(text: str) -> bool:
...
def validate_trace_section(path: str, text: str, *, usage_claimed: bool) -> list[Violation]:
    ...
    if usage_claimed and not section:
        return [
            Violation(
                path,
                f"provider-owned skill usage claim requires `{TRACE_HEADING}` block",
            )
        ]
    if not section:
        return []
    ...
    if usage_claimed and disposition != USED_TOKEN:
        violations.append(
            Violation(
                path,
                f"`Usage disposition` must be `{USED_TOKEN}` when provider skill use is claimed",
            )
        )
    ...
    if disposition == USED_TOKEN and "not CVF canonical authority" not in boundary:
        violations.append(
            Violation(
                path,
                "`Authority boundary` must state `not CVF canonical authority`",
            )
        )
    return violations
...
def check_text(path: str, text: str) -> list[Violation]:
    usage_claimed = has_provider_skill_usage_claim(text)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check external provider skill usage trace blocks"
    )
    ...
    print("=== CVF External Provider Skill Usage Trace Check ===")
    ...
    if violations:
        ...
        print("\nFAIL - external provider skill use is not traceable.")
        return 1 if args.enforce else 0

    print("PASS - external provider skill usage trace boundary is satisfied.")
    return 0
