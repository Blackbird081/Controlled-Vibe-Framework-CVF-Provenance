#!/usr/bin/env python3
"""
CVF External Absorption Overlap Discipline Guard.

Changed governed Markdown artifacts that absorb an external repo or copied
folder must compare incoming value against existing CVF owner surfaces before
opening new doctrine, package, runtime, or checker lanes. This is forward-only
and range-aware.
"""
...
THIS_SCRIPT_PATH = "governance/compat/check_external_absorption_overlap_discipline.py"

CORE_SECTION = "## External Absorption Core"
OVERLAP_SECTION = "## Overlap And Novelty Classification"
...
REQUIRED_COLUMNS = (
    ...
    "Existing CVF owner surface checked",
    "Overlap disposition",
    "Novelty / delta",
    "Action",
)
...
def _has_owner_surface(value: str) -> bool:
    if _is_empty_or_na(value):
        return False
    cleaned = _clean(value)
    return (
        "/" in cleaned
        or "OWNER_SURFACE_NOT_FOUND" in cleaned
        or "inline" in cleaned.casefold()
        or "existing governed" in cleaned.casefold()
    )


def _validate_overlap_section(path: str, section: str) -> list[dict[str, str]]:
    violations: list[dict[str, str]] = []
    rows = [row for row in _table_rows(section) if not _is_separator_row(row)]
    if not rows:
        return [{
            "path": path,
            "type": "external_absorption_overlap_table_missing",
            "message": f"`{OVERLAP_SECTION}` must contain a reviewable table",
        }]
    ...
    for column in REQUIRED_COLUMNS:
        if column.casefold() not in headers:
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_column_missing",
                "message": f"overlap classification missing column `{column}`",
            })
    if violations:
        return violations

    source_idx = headers["source item or group"]
    owner_idx = headers["existing cvf owner surface checked"]
    disposition_idx = headers["overlap disposition"]
    novelty_idx = headers["novelty / delta"]
    action_idx = headers["action"]

    body = rows[1:]
    row_count = 0
    disposition_text = "\n".join(row[disposition_idx] for row in body if len(row) > disposition_idx)
    if not any(token in disposition_text for token in ALLOWED_DISPOSITIONS):
        violations.append({
            "path": path,
            "type": "external_absorption_overlap_disposition_missing",
            "message": "overlap classification must include at least one allowed disposition token",
        })

    for row in body:
        if len(row) <= max(source_idx, owner_idx, disposition_idx, novelty_idx, action_idx):
            continue
        if row[disposition_idx].casefold() == "overlap disposition":
            continue
        row_count += 1
        source = row[source_idx]
        owner = row[owner_idx]
        disposition = row[disposition_idx]
        novelty = row[novelty_idx]
        action = row[action_idx]

        if _is_empty_or_na(source):
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_source_missing",
                "message": "overlap classification row must name a source item or group",
            })
        if not _has_owner_surface(owner):
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_owner_surface_missing",
                "message": "overlap classification must name an existing CVF owner surface checked or OWNER_SURFACE_NOT_FOUND",
            })
        unknown = [token for token in re.findall(r"\b[A-Z][A-Z0-9_]{3,}\b", disposition) if token not in ALLOWED_DISPOSITIONS]
        if unknown:
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_disposition_unknown",
                "message": f"overlap disposition contains unknown token(s): {', '.join(sorted(set(unknown)))}",
            })
        if not any(token in disposition for token in ALLOWED_DISPOSITIONS):
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_disposition_invalid",
                "message": f"overlap disposition must use one of: {', '.join(ALLOWED_DISPOSITIONS)}",
            })
        if _is_empty_or_na(novelty):
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_novelty_missing",
                "message": "overlap classification row must state the novelty or delta",
            })
        if _is_empty_or_na(action):
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_action_missing",
                "message": "overlap classification row must state the action",
            })

    if row_count == 0:
        violations.append({
            "path": path,
            "type": "external_absorption_overlap_no_rows",
            "message": "overlap classification must contain at least one body row",
        })
    ...
    for token in PLACEHOLDER_TOKENS:
        if token in lowered:
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_placeholder_residue",
                "message": "`Overlap And Novelty Classification` contains placeholder residue",
            })
            break
...
def _validate_standard(path: str, text: str) -> list[dict[str, str]]:
    ...
    for marker in required:
        if marker not in text:
            violations.append({
                "path": path,
                "type": "external_absorption_overlap_standard_marker_missing",
...
def check_text(path: str, text: str) -> list[dict[str, str]]:
    ...
    if OVERLAP_SECTION not in text:
        return [{
            "path": path,
            "type": "external_absorption_overlap_section_missing",
            "message": f"external absorption artifact must include `{OVERLAP_SECTION}`",
        }]
    return _validate_overlap_section(path, _extract_section(text, OVERLAP_SECTION))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="CVF External Absorption Overlap Discipline Guard")
    ...
    print("=== CVF External Absorption Overlap Discipline Guard ===")
    ...
    if result["violations"]:
        ...
        if args.enforce:
            print("VIOLATION - external absorption overlap discipline is incomplete.")
            return 1

    print("COMPLIANT - external absorption overlap discipline is aligned.")
    return 0
