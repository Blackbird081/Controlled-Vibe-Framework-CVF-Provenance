#!/usr/bin/env python3
"""
CVF Corpus Packet Source Hash Gate (NR-04)

Validates that corpus intelligence readiness packets include per-file
sourceHash fields in classification ledger rows, or declare an explicit
manifest proxy exception at packet level. Does not validate hash values
or semantic correctness.

Standard: docs/reference/CVF_CORPUS_SOURCE_HASH_STANDARD_2026-06-02.md
"""
...
SOURCE_HASH_STANDARD_PATH = "docs/reference/CVF_CORPUS_SOURCE_HASH_STANDARD_2026-06-02.md"
THIS_SCRIPT_PATH = "governance/compat/check_corpus_packet_source_hash.py"
...
# Columns required for a table to be considered a classification ledger
_CL_REQUIRED = {"sourcePath", "disposition"}
_CL_SPECIFIC = {"processingStatus", "knowledgeRegion", "ownerSurface", "evidencePointer"}

PLACEHOLDER_VALUES = {"", "n/a", "none", "tbd", "todo", "pending", "-"}
...
def validate_source_hash(path: str, text: str) -> list[dict[str, str]]:
    """Validate NR-04 source hash discipline in a readiness packet.

    Public interface for tests.
    """
    violations: list[dict[str, str]] = []

    # Check for packet-level manifest proxy exception
    has_proxy = bool(re.search(r"manifestHashProxy\s*:\s*true", text, re.IGNORECASE))
    if has_proxy:
        exc_match = re.search(r"manifestProxyException\s*[:\-]\s*(.+)", text, re.IGNORECASE)
        exc_val = exc_match.group(1).strip() if exc_match else ""
        if len(exc_val) < 20:
            _add(
                violations, path, "manifest_proxy_exception_missing",
                "manifestHashProxy: true declared but manifestProxyException is absent or insufficient; "
                "must state reason and coverage limitation per NR-04-B",
            )
        return violations

    # No proxy — validate classification ledger rows
    ...
    for table in ledger_tables:
        cols = set(table[0].keys())
        if "sourceHash" not in cols:
            _add(
                violations, path, "source_hash_column_missing",
                "classification ledger lacks sourceHash column; per NR-04-A every row must include "
                "sourceHash (SHA-256 hex lowercase of raw source file bytes), or declare "
                "manifestHashProxy: true with manifestProxyException at packet level",
            )
            continue
        for idx, row in enumerate(table, start=1):
            val = _strip_val(row.get("sourceHash", "")).lower()
            if val in PLACEHOLDER_VALUES:
                _add(
                    violations, path, "source_hash_empty",
                    f"classification ledger row {idx} (sourcePath={row.get('sourcePath', '?')!r}) "
                    "has empty or placeholder sourceHash; NR-04-A requires a non-empty SHA-256 hex value",
                )
...
def _validate_path(path: str) -> list[dict[str, str]]:
    ...
    if not _is_applicable(path, text):
        return []
    return validate_source_hash(path, text)
...
def _classify(changed_paths: dict[str, list[str]]) -> dict[str, Any]:
    ...
    for path, statuses in sorted(changed_paths.items()):
        ...
        if not _is_applicable(path, text):
            continue
        checked.append(path)
        violations.extend(validate_source_hash(path, text))
    return {
...
def _print_report(report: dict[str, Any], base: str, head: str, range_source: str) -> None:
    print("=== CVF Corpus Packet Source Hash Gate (NR-04) ===")
    print(f"Range: {base}..{head} ({range_source})")
    print(f"Standard: {SOURCE_HASH_STANDARD_PATH}")
    ...
    if report["compliant"]:
        print("\nCOMPLIANT - corpus packet source hash discipline satisfied.")
    else:
        print("\nVIOLATION - corpus packet source hash discipline incomplete.")


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate corpus packet source hash (NR-04)")
