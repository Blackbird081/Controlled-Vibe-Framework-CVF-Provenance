/**
 * Deterministic dependency boundary. RefineryEngine accepts these as
 * constructor injection; no stage or helper may call a global wall-clock
...
export class DeterministicClock implements Clock {
  ...
  constructor(startUtcIso: string, stepMs = 0) {
    ...
    if (Number.isNaN(this.cursor)) {
      throw new Error(`REFINERY_INVALID_CLOCK_START: ${startUtcIso}`);
    }
    this.stepMs = stepMs;
  }
