/**
 * w137-post-hardening-stability.live.spec.ts
 * W137-T1 CP2 + CP3 — Post-Hardening Full Matrix Re-Run
 *
 ...
 *     npx playwright test tests/e2e/w137-post-hardening-stability.live.spec.ts
 *
 * Outputs:
 *   docs/reviews/CVF_W137_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.md
 *   docs/reviews/CVF_W137_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.json
 *   docs/reviews/CVF_W137_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.md
...
const ALIBABA_MD = resolve(EVIDENCE_ROOT, 'CVF_W137_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.md');
const ALIBABA_JSON = resolve(EVIDENCE_ROOT, 'CVF_W137_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.json');
const DEEPSEEK_MD = resolve(EVIDENCE_ROOT, 'CVF_W137_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.md');
const DEEPSEEK_JSON = resolve(EVIDENCE_ROOT, 'CVF_W137_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.json');
...
function writeEvidence(
  ...
  const md = [
    ...
    `# CVF W137 Runtime Stability — ${providerKey === 'alibaba' ? 'Alibaba Primary' : 'DeepSeek Confirmatory'}`,
    '',
...
async function loginForW137(page: Page) {
  ...
  try {
    ...
    for (let i = 0; i < taCount; i++) {
      ...
      if (await ta.isVisible().catch(() => false)) {
        ...
        if (!v) await ta.fill('Nội dung kiểm thử W137 cho trường này.');
      }
    }
...
// ── W137 CP2 — Alibaba post-hardening full matrix ────────────────────────────

test.describe('W137-T1 CP2 — Alibaba post-hardening full matrix', () => {
  test(
    'combined W134/W135/W136 hardening; ≥12 attempted ≥11 accepted; prior blocker classes absent',
    { tag: ['@live', '@w137-cp2', '@w137-alibaba'] },
    async ({ browser }) => {
      test.skip(!HAS_ALIBABA_KEY, 'W137 CP2 requires DASHSCOPE_API_KEY / ALIBABA_API_KEY');
      test.skip(!INTENT_FIRST_ENABLED, 'W137 CP2 requires NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR=true');
      test.skip(!CLARIFICATION_LOOP_ENABLED, 'W137 CP2 requires NEXT_PUBLIC_CVF_NONCODER_CLARIFICATION_LOOP=true');
      test.skip(!ITERATION_MEMORY_ENABLED, 'W137 CP2 requires NEXT_PUBLIC_CVF_NONCODER_ITERATION_MEMORY=true');
      ...
      for (const journey of JOURNEY_MATRIX) {
        ...
        console.log(`[W137-alibaba] journey ${idx}/${JOURNEY_MATRIX.length}: ${journey.formType}`);

        const ctx = await browser.newContext();
        const page = await ctx.newPage();
        try {
          await setupPage(page, 'alibaba');
          await loginForW137(page);
          const result = await runIsolatedJourney(page, journey, config);
          journeyLog.push(result);
          console.log(
            `[W137-alibaba] outcome: ${result.outcome}  subcode: ${result.diagnosticSubcode ?? 'none'}  elapsed: ${result.elapsedMs}ms  http: ${result.httpStatus ?? 'none'}`,
          );
        } catch (err) {
          journeyLog.push({
            ...config,
          ...
          console.log(`[W137-alibaba] context crash: ${String(err).slice(0, 100)}`);
        } finally {
        ...
        writeEvidence(
          'alibaba', 'qwen-plus',
          'W137-T1 CP2 — Alibaba Post-Hardening Full Matrix',
        ...
        if (idx < JOURNEY_MATRIX.length) {
          console.log(`[W137-alibaba] inter-journey delay 1500ms`);
      ...
      console.log('[W137-alibaba] attempted:', summary.attemptedJourneys);
      console.log('[W137-alibaba] accepted:', summary.acceptedWithReceipt);
      console.log('[W137-alibaba] failure rate:', summary.failureRate);
      console.log('[W137-alibaba] outcomes:', JSON.stringify(summary.outcomeBreakdown));
      console.log('[W137-alibaba] diagnostics:', JSON.stringify(summary.diagnosticBreakdown));
...
// ── W137 CP3 — DeepSeek confirmatory full matrix ─────────────────────────────

test.describe('W137-T1 CP3 — DeepSeek confirmatory full matrix', () => {
  test(
    'combined W134/W135/W136 hardening; ≥6 attempted 6 accepted',
    { tag: ['@live', '@w137-cp3', '@w137-deepseek'] },
    async ({ browser }) => {
      test.skip(!HAS_DEEPSEEK_KEY, 'W137 CP3 requires DEEPSEEK_API_KEY');
      test.skip(!INTENT_FIRST_ENABLED, 'W137 CP3 requires NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR=true');
      test.skip(!CLARIFICATION_LOOP_ENABLED, 'W137 CP3 requires NEXT_PUBLIC_CVF_NONCODER_CLARIFICATION_LOOP=true');
      test.skip(!ITERATION_MEMORY_ENABLED, 'W137 CP3 requires NEXT_PUBLIC_CVF_NONCODER_ITERATION_MEMORY=true');
      ...
      for (const journey of DEEPSEEK_JOURNEYS) {
        const idx = journeyLog.length + 1;
        console.log(`[W137-deepseek] journey ${idx}/${DEEPSEEK_JOURNEYS.length}: ${journey.formType}`);
        ...
        try {
          ...
          journeyLog.push(result);
          console.log(
            `[W137-deepseek] outcome: ${result.outcome}  subcode: ${result.diagnosticSubcode ?? 'none'}  elapsed: ${result.elapsedMs}ms`,
          );
        } catch (err) {
          journeyLog.push({
            ...config,
          ...
          console.log(`[W137-deepseek] context crash: ${String(err).slice(0, 100)}`);
        } finally {
          await ctx.close().catch(() => {});
        }
        ...
        writeEvidence(
          ...
          'W137-T1 CP3 — DeepSeek Confirmatory Full Matrix',
          journeyLog, 0, null, 'deepseek', 'in_progress',
        );

        // Fix-C: inter-journey delay
        if (idx < DEEPSEEK_JOURNEYS.length) {
          console.log(`[W137-deepseek] inter-journey delay 1500ms`);
          await new Promise((r) => setTimeout(r, 1500));
        }
      }
      ...
      console.log('[W137-deepseek] attempted:', summary.attemptedJourneys);
      console.log('[W137-deepseek] accepted:', summary.acceptedWithReceipt);
      console.log('[W137-deepseek] outcomes:', JSON.stringify(summary.outcomeBreakdown));
      console.log('[W137-deepseek] diagnostics:', JSON.stringify(summary.diagnosticBreakdown));
