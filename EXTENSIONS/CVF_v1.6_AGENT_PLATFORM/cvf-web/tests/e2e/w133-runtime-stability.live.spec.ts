/**
 * w133-runtime-stability.live.spec.ts
 * W133-T1 CP4 + CP5 — Sequential Journey Stability Re-Run After Connection Lifecycle Fix
 *
 ...
 *
 * Fix C (this file): 1500ms inter-journey delay between isolated contexts
 ...
 *     npx playwright test tests/e2e/w133-runtime-stability.live.spec.ts
 *
 * Outputs:
 *   docs/reviews/CVF_W133_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.md
 *   docs/reviews/CVF_W133_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.json
 *   docs/reviews/CVF_W133_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.md
 *   docs/reviews/CVF_W133_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.json
 *
 * W133-T1 CP4+CP5
 */
...
const ALIBABA_MD = resolve(EVIDENCE_ROOT, 'CVF_W133_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.md');
const ALIBABA_JSON = resolve(EVIDENCE_ROOT, 'CVF_W133_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.json');
const DEEPSEEK_MD = resolve(EVIDENCE_ROOT, 'CVF_W133_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.md');
const DEEPSEEK_JSON = resolve(EVIDENCE_ROOT, 'CVF_W133_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.json');
...
function writeEvidence(
  ...
  const summary = {
    ...
    model,
    tranche,
    fixes: ['Fix-A: Connection close + 60s timeout', 'Fix-B: form-first routing', 'Fix-C: 1500ms inter-journey delay'],
    flags: {
      NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR: true,
      NEXT_PUBLIC_CVF_NONCODER_CLARIFICATION_LOOP: true,
      NEXT_PUBLIC_CVF_NONCODER_ITERATION_MEMORY: true,
  ...
  const md = [
    ...
    `# CVF W133 Runtime Stability — ${providerKey === 'alibaba' ? 'Alibaba Primary' : 'DeepSeek Confirmatory'}`,
    '',
...
async function loginForW133(page: Page) {
  ...
  try {
    ...
    for (let i = 0; i < taCount; i++) {
      ...
      if (await ta.isVisible().catch(() => false)) {
        ...
        if (!v) await ta.fill('Nội dung kiểm thử W133 cho trường này.');
      }
    }
...
// ── W133 CP4 — Alibaba isolated-session stability run ────────────────────────

test.describe('W133-T1 CP4 — Alibaba isolated-session stability run', () => {
  test(
    'Fix-A/B/C applied; isolated context per journey; 1500ms inter-journey delay; ≥12 attempted ≥8 accepted',
    { tag: ['@live', '@w133-cp4', '@w133-alibaba'] },
    async ({ browser }) => {
      test.skip(!HAS_ALIBABA_KEY, 'W133 CP4 requires DASHSCOPE_API_KEY / ALIBABA_API_KEY');
      test.skip(!INTENT_FIRST_ENABLED, 'W133 CP4 requires NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR=true');
      test.skip(!CLARIFICATION_LOOP_ENABLED, 'W133 CP4 requires NEXT_PUBLIC_CVF_NONCODER_CLARIFICATION_LOOP=true');
      test.skip(!ITERATION_MEMORY_ENABLED, 'W133 CP4 requires NEXT_PUBLIC_CVF_NONCODER_ITERATION_MEMORY=true');
      ...
      for (const journey of JOURNEY_MATRIX) {
        ...
        console.log(`[W133-alibaba] journey ${idx}/${JOURNEY_MATRIX.length}: ${journey.formType}`);

        const ctx = await browser.newContext();
        const page = await ctx.newPage();
        try {
          await setupPage(page, 'alibaba');
          await loginForW133(page);
          const result = await runIsolatedJourney(page, journey, config);
          journeyLog.push(result);
          console.log(
            `[W133-alibaba] outcome: ${result.outcome}  subcode: ${result.diagnosticSubcode ?? 'none'}  elapsed: ${result.elapsedMs}ms  http: ${result.httpStatus ?? 'none'}`,
          );
        } catch (err) {
          journeyLog.push({
            ...config,
          ...
          console.log(`[W133-alibaba] context crash: ${String(err).slice(0, 100)}`);
        } finally {
        ...
        writeEvidence(
          'alibaba', 'qwen-plus',
          'W133-T1 CP4 — Alibaba Isolated Stability Run',
          journeyLog, 0, null, 'alibaba', 'in_progress',
        );

        // Fix-C: inter-journey delay — give OS TCP stack time to flush connection
        if (idx < JOURNEY_MATRIX.length) {
          console.log(`[W133-alibaba] inter-journey delay 1500ms`);
      ...
      console.log('[W133-alibaba] attempted:', summary.attemptedJourneys);
      console.log('[W133-alibaba] accepted:', summary.acceptedWithReceipt);
      console.log('[W133-alibaba] failure rate:', summary.failureRate);
      console.log('[W133-alibaba] outcomes:', JSON.stringify(summary.outcomeBreakdown));
      console.log('[W133-alibaba] diagnostics:', JSON.stringify(summary.diagnosticBreakdown));
...
// ── W133 CP5 — DeepSeek isolated-session confirmatory run ────────────────────

test.describe('W133-T1 CP5 — DeepSeek isolated-session confirmatory run', () => {
  test(
    'Fix-A/B/C applied; isolated context per journey; 1500ms inter-journey delay; ≥6 attempted ≥3 accepted',
    { tag: ['@live', '@w133-cp5', '@w133-deepseek'] },
    async ({ browser }) => {
      test.skip(!HAS_DEEPSEEK_KEY, 'W133 CP5 requires DEEPSEEK_API_KEY');
      test.skip(!INTENT_FIRST_ENABLED, 'W133 CP5 requires NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR=true');
      test.skip(!CLARIFICATION_LOOP_ENABLED, 'W133 CP5 requires NEXT_PUBLIC_CVF_NONCODER_CLARIFICATION_LOOP=true');
      test.skip(!ITERATION_MEMORY_ENABLED, 'W133 CP5 requires NEXT_PUBLIC_CVF_NONCODER_ITERATION_MEMORY=true');
      ...
      for (const journey of DEEPSEEK_JOURNEYS) {
        const idx = journeyLog.length + 1;
        console.log(`[W133-deepseek] journey ${idx}/${DEEPSEEK_JOURNEYS.length}: ${journey.formType}`);
        ...
        try {
          ...
          journeyLog.push(result);
          console.log(
            `[W133-deepseek] outcome: ${result.outcome}  subcode: ${result.diagnosticSubcode ?? 'none'}  elapsed: ${result.elapsedMs}ms`,
          );
        } catch (err) {
          journeyLog.push({
            ...config,
          ...
          console.log(`[W133-deepseek] context crash: ${String(err).slice(0, 100)}`);
        } finally {
          await ctx.close().catch(() => {});
        }
      ...
      console.log('[W133-deepseek] attempted:', summary.attemptedJourneys);
      console.log('[W133-deepseek] accepted:', summary.acceptedWithReceipt);
      console.log('[W133-deepseek] outcomes:', JSON.stringify(summary.outcomeBreakdown));
      console.log('[W133-deepseek] diagnostics:', JSON.stringify(summary.diagnosticBreakdown));
