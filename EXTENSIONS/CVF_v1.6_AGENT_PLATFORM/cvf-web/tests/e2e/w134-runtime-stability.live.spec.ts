/**
 * w134-runtime-stability.live.spec.ts
 * W134-T1 CP4 + CP5 — Runtime Stability Re-Run After Pre-AI HTTP 400 Fix
 *
 ...
 *     npx playwright test tests/e2e/w134-runtime-stability.live.spec.ts
 *
 * Outputs:
 *   docs/reviews/CVF_W134_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.md
 *   docs/reviews/CVF_W134_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.json
 *   docs/reviews/CVF_W134_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.md
...
const ALIBABA_MD = resolve(EVIDENCE_ROOT, 'CVF_W134_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.md');
const ALIBABA_JSON = resolve(EVIDENCE_ROOT, 'CVF_W134_RUNTIME_STABILITY_ALIBABA_EVIDENCE_2026-05-07.json');
const DEEPSEEK_MD = resolve(EVIDENCE_ROOT, 'CVF_W134_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.md');
const DEEPSEEK_JSON = resolve(EVIDENCE_ROOT, 'CVF_W134_RUNTIME_STABILITY_DEEPSEEK_EVIDENCE_2026-05-07.json');
...
function writeEvidence(
  ...
  const summary = {
    ...
    fixes: [
      ...
      'Fix-C: 1500ms inter-journey delay',
  ...
  const md = [
    ...
    `# CVF W134 Runtime Stability — ${providerKey === 'alibaba' ? 'Alibaba Primary' : 'DeepSeek Confirmatory'}`,
    '',
...
async function loginForW134(page: Page) {
  ...
  try {
    ...
    for (let i = 0; i < taCount; i++) {
      ...
      if (await ta.isVisible().catch(() => false)) {
        ...
        if (!v) await ta.fill('Nội dung kiểm thử W134 cho trường này.');
      }
    }
...
// ── W134 CP4 — Alibaba isolated-session stability run ────────────────────────

test.describe('W134-T1 CP4 — Alibaba isolated-session stability run', () => {
  test(
    'Fix-A/B/C applied; isolated context per journey; 1500ms inter-journey delay; ≥12 attempted ≥8 accepted',
    { tag: ['@live', '@w134-cp4', '@w134-alibaba'] },
    async ({ browser }) => {
      test.skip(!HAS_ALIBABA_KEY, 'W134 CP4 requires DASHSCOPE_API_KEY / ALIBABA_API_KEY');
      test.skip(!INTENT_FIRST_ENABLED, 'W134 CP4 requires NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR=true');
      test.skip(!CLARIFICATION_LOOP_ENABLED, 'W134 CP4 requires NEXT_PUBLIC_CVF_NONCODER_CLARIFICATION_LOOP=true');
      test.skip(!ITERATION_MEMORY_ENABLED, 'W134 CP4 requires NEXT_PUBLIC_CVF_NONCODER_ITERATION_MEMORY=true');
      ...
      for (const journey of JOURNEY_MATRIX) {
        ...
        console.log(`[W134-alibaba] journey ${idx}/${JOURNEY_MATRIX.length}: ${journey.formType}`);

        const ctx = await browser.newContext();
        const page = await ctx.newPage();
        try {
          await setupPage(page, 'alibaba');
          await loginForW134(page);
          const result = await runIsolatedJourney(page, journey, config);
          journeyLog.push(result);
          console.log(
            `[W134-alibaba] outcome: ${result.outcome}  subcode: ${result.diagnosticSubcode ?? 'none'}  elapsed: ${result.elapsedMs}ms  http: ${result.httpStatus ?? 'none'}`,
          );
        } catch (err) {
          journeyLog.push({
            ...config,
          ...
          console.log(`[W134-alibaba] context crash: ${String(err).slice(0, 100)}`);
        } finally {
        ...
        writeEvidence(
          'alibaba', 'qwen-plus',
          'W134-T1 CP4 — Alibaba Isolated Stability Run',
        ...
        if (idx < JOURNEY_MATRIX.length) {
          console.log(`[W134-alibaba] inter-journey delay 1500ms`);
      ...
      console.log('[W134-alibaba] attempted:', summary.attemptedJourneys);
      console.log('[W134-alibaba] accepted:', summary.acceptedWithReceipt);
      console.log('[W134-alibaba] failure rate:', summary.failureRate);
      console.log('[W134-alibaba] outcomes:', JSON.stringify(summary.outcomeBreakdown));
      console.log('[W134-alibaba] diagnostics:', JSON.stringify(summary.diagnosticBreakdown));

      expect(summary.attemptedJourneys).toBeGreaterThanOrEqual(12);
      expect(summary.acceptedWithReceipt).toBeGreaterThanOrEqual(8);
...
// ── W134 CP5 — DeepSeek isolated-session confirmatory run ────────────────────

test.describe('W134-T1 CP5 — DeepSeek isolated-session confirmatory run', () => {
  test(
    'Fix-A/B/C applied; isolated context per journey; 1500ms inter-journey delay; ≥6 attempted ≥3 accepted',
    { tag: ['@live', '@w134-cp5', '@w134-deepseek'] },
    async ({ browser }) => {
      test.skip(!HAS_DEEPSEEK_KEY, 'W134 CP5 requires DEEPSEEK_API_KEY');
      test.skip(!INTENT_FIRST_ENABLED, 'W134 CP5 requires NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR=true');
      test.skip(!CLARIFICATION_LOOP_ENABLED, 'W134 CP5 requires NEXT_PUBLIC_CVF_NONCODER_CLARIFICATION_LOOP=true');
      test.skip(!ITERATION_MEMORY_ENABLED, 'W134 CP5 requires NEXT_PUBLIC_CVF_NONCODER_ITERATION_MEMORY=true');
      ...
      for (const journey of DEEPSEEK_JOURNEYS) {
        const idx = journeyLog.length + 1;
        console.log(`[W134-deepseek] journey ${idx}/${DEEPSEEK_JOURNEYS.length}: ${journey.formType}`);
        ...
        try {
          ...
          journeyLog.push(result);
          console.log(
            `[W134-deepseek] outcome: ${result.outcome}  subcode: ${result.diagnosticSubcode ?? 'none'}  elapsed: ${result.elapsedMs}ms`,
          );
        } catch (err) {
          journeyLog.push({
            ...config,
          ...
          console.log(`[W134-deepseek] context crash: ${String(err).slice(0, 100)}`);
        } finally {
          await ctx.close().catch(() => {});
        }
      ...
      console.log('[W134-deepseek] attempted:', summary.attemptedJourneys);
      console.log('[W134-deepseek] accepted:', summary.acceptedWithReceipt);
      console.log('[W134-deepseek] outcomes:', JSON.stringify(summary.outcomeBreakdown));
      console.log('[W134-deepseek] diagnostics:', JSON.stringify(summary.diagnosticBreakdown));

      expect(summary.attemptedJourneys).toBeGreaterThanOrEqual(6);
      expect(summary.acceptedWithReceipt).toBeGreaterThanOrEqual(3);
    },
  );
});
