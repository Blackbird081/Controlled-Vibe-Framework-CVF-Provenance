...
Authorized scope: enforce source-derived closure eligibility without changing Local/external roles. Protected paths: `governance/compat/check_external_knowledge_intake_routing.py`; `governance/compat/test_check_external_knowledge_intake_routing.py`.
Operator authorization: explicit 2026-09-25 direction to correct coordination rules before WikiSkill P0 and HyperFrames P0.
Rollback boundary: revert only the two method edits and checker/test correction; preserve pinned-source intake, Jev P0, G1-G7 posture, and unrelated history.
