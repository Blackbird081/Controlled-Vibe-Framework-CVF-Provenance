...
function Test-CvfByteArrayEqual([byte[]]$Left, [byte[]]$Right) {
...
function Get-CvfMergeBlockReadout([byte[]]$Bytes) {
    $inspection = [System.Text.Encoding]::GetEncoding(28591).GetString($Bytes)
    $startToken = '<!-- CVF_MERGE_BLOCK_START:'
    $endToken = '<!-- CVF_MERGE_BLOCK_END -->'
    $starts = [regex]::Matches($inspection, [regex]::Escape($startToken))
    $ends = [regex]::Matches($inspection, [regex]::Escape($endToken))
    if ($starts.Count -ne 1 -or $ends.Count -ne 1 -or $starts[0].Index -ge $ends[0].Index) {
        return [PSCustomObject]@{ Valid = $false; Count = $starts.Count; BlockText = ""; OutsideBytes = [byte[]]@() }
    }
    $blockEnd = $ends[0].Index + $endToken.Length
    if ($inspection.Substring($blockEnd).StartsWith("`r`n`r`n")) { $blockEnd += 4 }
    elseif ($inspection.Substring($blockEnd).StartsWith("`n`n")) { $blockEnd += 2 }
    $outside = [System.IO.MemoryStream]::new()
    $outside.Write($Bytes, 0, $starts[0].Index)
    $outside.Write($Bytes, $blockEnd, $Bytes.Length - $blockEnd)
    $outsideBytes = $outside.ToArray()
    $outside.Dispose()
    return [PSCustomObject]@{
        Valid = $true
        Count = $starts.Count
        BlockText = $inspection.Substring($starts[0].Index, $blockEnd - $starts[0].Index)
        OutsideBytes = $outsideBytes
    }
}
function Test-CvfCarrierContent([string]$Text) {
    return (([regex]::Matches($Text, [regex]::Escape('### Governance Latency and Approval Continuity'))).Count -eq 1) -and
        ($Text -match 'dependent same-scope repairs') -and
        ($Text -match 'real boundary change') -and
        ($Text -match 'consolidated review of relevant records') -and
        ($Text -match 'REVIEW_COST_ESCALATION_REQUIRED') -and
        ($Text -match 'avoidable operator wait')
}
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $repoRoot "governance\toolkit\05_OPERATION\CVF_DOWNSTREAM_AGENTS_TEMPLATE.md") -Destination (Join-Path $coreA "governance\toolkit\05_OPERATION\CVF_DOWNSTREAM_AGENTS_TEMPLATE.md") -Force
    ...
    $generatedAgentsPath = Join-Path $projectA "AGENTS.md"
    $generatedAgentsFirst = Get-Content -LiteralPath $generatedAgentsPath -Raw
    Add-Result "GLP-T2R1" "Fresh bootstrap projects exactly one complete governance-latency carrier" (Test-CvfCarrierContent $generatedAgentsFirst)
    # AC-02: active state
    ...
    $generatedAgentsSecond = Get-Content -LiteralPath $generatedAgentsPath -Raw
    Add-Result "GLP-T2R1" "Second bootstrap retains exactly one complete governance-latency carrier" (Test-CvfCarrierContent $generatedAgentsSecond)
    ...
    $sentinelAgentsPath = Join-Path $sentinelProject "AGENTS.md"
    $sentinelAgentsBytes = [System.Text.Encoding]::ASCII.GetBytes("# PROJECT OWNED AGENTS`r`n`r`nKeep CRLF here.`r`nKeep final LF here.`n")
    [System.IO.File]::WriteAllBytes($sentinelAgentsPath, $sentinelAgentsBytes)
    ...
    $sentinelFirstReadout = Get-CvfMergeBlockReadout ([System.IO.File]::ReadAllBytes($sentinelAgentsPath))
    Add-Result "GLP-T2R1" "Hand-edited AGENTS first insertion has one merge block and the complete carrier" `
        ($sentinelFirstReadout.Valid -and (Test-CvfCarrierContent $sentinelFirstReadout.BlockText))
    Add-Result "GLP-T2R1" "Hand-edited AGENTS first insertion preserves all project-owned bytes" `
        (Test-CvfByteArrayEqual $sentinelAgentsBytes $sentinelFirstReadout.OutsideBytes) "equivalence=MATCH"
    $sentinelSecondBootstrap = Invoke-CvfBootstrap -WorkspaceRoot $workspaceA -ProjectName $sentinelProjectName -BootstrapScriptPath $bootstrapScript
    $sentinelSecondReadout = Get-CvfMergeBlockReadout ([System.IO.File]::ReadAllBytes($sentinelAgentsPath))
    Add-Result "GLP-T2R1" "Hand-edited AGENTS refresh exits 0 and retains one complete merge block" `
        (($sentinelSecondBootstrap.ExitCode -eq 0) -and $sentinelSecondReadout.Valid -and (Test-CvfCarrierContent $sentinelSecondReadout.BlockText)) $sentinelSecondBootstrap.Output
    Add-Result "GLP-T2R1" "Hand-edited AGENTS refresh preserves all project-owned bytes" `
        (Test-CvfByteArrayEqual $sentinelAgentsBytes $sentinelSecondReadout.OutsideBytes) "equivalence=MATCH"
    $malformedCases = @(
        @{ Name = "duplicate"; Content = "<!-- CVF_MERGE_BLOCK_START: one -->`r`n<!-- CVF_MERGE_BLOCK_START: two -->`r`n<!-- CVF_MERGE_BLOCK_END -->`r`nPROJECT" },
        @{ Name = "reversed"; Content = "<!-- CVF_MERGE_BLOCK_END -->`r`n<!-- CVF_MERGE_BLOCK_START: late -->`r`nPROJECT" },
        @{ Name = "unterminated"; Content = "<!-- CVF_MERGE_BLOCK_START: open -->`r`nPROJECT" }
    )
    foreach ($case in $malformedCases) {
        $malformedProject = Copy-DisposableProject -SourceProjectPath $sentinelProject -WorkspaceRoot $workspaceA -Label "malformed-$($case.Name)"
        $malformedAgentsPath = Join-Path $malformedProject "AGENTS.md"
        $beforeMalformed = [System.Text.Encoding]::ASCII.GetBytes($case.Content)
        [System.IO.File]::WriteAllBytes($malformedAgentsPath, $beforeMalformed)
        $malformedResult = Invoke-CvfBootstrap -WorkspaceRoot $workspaceA -ProjectName (Split-Path -Leaf $malformedProject) -BootstrapScriptPath $bootstrapScript
        $afterMalformed = [System.IO.File]::ReadAllBytes($malformedAgentsPath)
        Add-Result "GLP-T2R1" "Malformed $($case.Name) merge markers fail closed without AGENTS mutation" `
            (($malformedResult.ExitCode -ne 0) -and ($malformedResult.Output -match "MALFORMED_CVF_MERGE_BLOCK") -and (Test-CvfByteArrayEqual $beforeMalformed $afterMalformed)) $malformedResult.Output
        Remove-CvfHermeticDirectory -Path $malformedProject | Out-Null
    }
    $privateSentinels = @(
        'SOT3-T2', 'WS2-T1', 'UNG DUNG AI\TOOL AI 2026',
        'REVIEW_CHANGES_REQUIRED with F1-F7', 'operator/session identity',
        '14/15', '93%', '8-10/15', '28 R2'
    )
    $carrierScanPaths = @(
        (Join-Path $repoRoot "governance\toolkit\05_OPERATION\CVF_DOWNSTREAM_AGENTS_TEMPLATE.md"),
        $generatedAgentsPath,
        $sentinelAgentsPath
    )
    $privateHits = [System.Collections.Generic.List[string]]::new()
    foreach ($scanPath in $carrierScanPaths) {
        $scanText = Get-Content -LiteralPath $scanPath -Raw
        foreach ($privateSentinel in $privateSentinels) {
            if ($scanText.Contains($privateSentinel)) { $privateHits.Add("$scanPath::$privateSentinel") }
        }
    }
    Add-Result "GLP-T2R1" "Template and generated/merge-path AGENTS files contain no private sentinels" ($privateHits.Count -eq 0) ($privateHits -join ", ")
