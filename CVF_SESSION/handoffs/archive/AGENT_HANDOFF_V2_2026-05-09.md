...
QBS-9 reviewer scoring and agreement:

- Operator asked "next" after QBS8; continuation proceeded without pausing for
  incremental approvals under the earlier authorization to complete benchmark
  work and use Alibaba/OpenAI/DeepSeek keys without cost concern.
- GC-018:
  `docs/reference/CVF_GC018_QBS9_REVIEWER_SCORING_CANDIDATE_2026-05-10.md`
- Roadmap:
  `docs/roadmaps/CVF_QBS9_REVIEWER_SCORING_ROADMAP_2026-05-10.md`
...
- Why R5 was created:
  R4 hard gates passed but only retained output previews and hashes. QBS9 could
  not score output quality from previews, so R5 reran the same contract with
  `--retain-redacted-outputs` to publish
  `redacted-reviewer-output-bundle.json`.
- R5 live execution:
  48 tasks x 3 repeats x 3 configs = 432 configuration executions on
  Alibaba/DashScope `qwen-turbo`.
- R5 initial run:
  one transient validation false block on `QBS1-F3-T03` repeat 1.
- R5 resume:
  `--resume-missing-cfg-b` reran mismatched/missing `CFG-B` rows only and
  produced final hard-gate PASS.
- Final R5 hard gates:
  - direct configs transport OK: true
  - `CFG-B` receipt completeness: true
  - `CFG-B` expected decision match: true
...
- Reviewer scoring:
  ...
  - 96 total reviewer calls, one task per call, 9 blinded outputs per task.
- Reviewer agreement:
...
- Validation:
  ...
  - targeted raw-key scan over public benchmark/evidence/scripts/QBS code: no
    matches.
- Boundary:
  QBS9 is a scored no-claim result. Reviewer agreement passed, but the measured
  quality delta was negative, so no public QBS score, L4/L5/L6,
  family-level claim, or provider parity claim is allowed.
- Recommended next track:
  `QBS10-QUALITY-DELTA-ROOT-CAUSE-AND-IMPROVEMENT`, focused on diagnosing why
  `CFG-B` underperformed `CFG-A1` and designing bounded improvements before any
  future scored claim attempt.
